...
console.log(`  patient_id: ${beforeOrder.patient_id}`);
console.log();

// 2. 返金処理を適用
console.log("【2. 返金処理を適用中...】");
...
console.log(`  updated_at: ${updatedOrder.updated_at}`);
console.log();
...
try {
  ...
  console.log("ローカル環境が起動していない可能性があります。本番環境でも実行してください。");
}
...
const { data: finalOrder, error: finalError } = await supabase
  .from("orders")
  .select("id, status, refunded_at, updated_at")
  .eq("id", paymentId)
  .single();
...
if (finalError) {
  ...
  console.log(`  updated_at: ${finalOrder.updated_at}`);
}

console.log();
console.log("=== 完了 ===");
console.log("マイページで返金が正しく表示されるか確認してください。");
