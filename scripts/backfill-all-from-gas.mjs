...
console.log('=== Backfilling ALL patients from GAS ===');
console.log('URL:', gasIntakeListUrl);
console.log('');

// Load patient IDs
const patientIdsFile = join(__dirname, 'missing-tel-kana-patient-ids.txt');
...
try {
  ...
  for (let i = 0; i < patientIds.length; i++) {
    ...
    if (i % 50 === 0) {
      console.log(`\n[${i + 1}/${patientIds.length}] Processing...`);
    }
    ...
    if (!gasIntake) {
      if (i < 10) console.log(`  ${patientId}: ❌ NOT FOUND in GAS`);
      notFoundCount++;
      continue;
    }
    ...
    if (!tel && !kana) {
      if (i < 10) console.log(`  ${patientId}: ⚠️ No tel/kana in GAS either`);
      noDataCount++;
      continue;
    }
    ...
    if (intakeError || !intake) {
      if (i < 10) console.log(`  ${patientId}: ❌ Intake not found in Supabase`);
      errorCount++;
      continue;
    }
    ...
    if (updateError) {
      if (i < 10) console.log(`  ${patientId}: ❌ Update error: ${updateError.message}`);
      errorCount++;
    } else {
      if (i < 10) console.log(`  ${patientId}: ✅ Updated`);
      successCount++;
    }
  }
