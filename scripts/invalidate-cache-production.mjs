// scripts/invalidate-cache-production.mjs
// 本番環境のキャッシュ無効化
...
// ★ 本番URLを指定（コマンドライン引数またはデフォルト）
const productionUrl = process.argv[2] || "https://em-clinic.vercel.app";
...
async function invalidateCache() {
  ...
  try {
    ...
    if (error.message.includes("fetch failed") || error.message.includes("ENOTFOUND")) {
      console.log("\n原因: 本番URLに接続できません");
      console.log(`確認: ${productionUrl} が正しい本番URLですか？`);
      console.log("\n使い方: node scripts/invalidate-cache-production.mjs https://your-app.vercel.app");
    }
  }
}
