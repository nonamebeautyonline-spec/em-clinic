// scripts/test-intake-reservation-0206.mjs
// 問診送信→予約作成テスト（2026-02-06 12:00指定）
...
const reserveDate = "2026-02-06";
const reserveTime = "12:00";

console.log("=== 問診送信→予約作成フローテスト ===\n");
console.log(`テスト用 patient_id: ${testPatientId}`);
console.log(`予約日時: ${reserveDate} ${reserveTime}\n`);
...
async function testFlow() {
  ...
  try {
    ...
    console.log(`  Response: ${JSON.stringify(intakeJson)}`);
  ...
  if (intakeError) {
    console.log(`  ❌ Supabaseクエリエラー: ${intakeError.message}\n`);
    return;
  } else if (!intakeData) {
    ...
    console.log(`      answers.カナ: ${intakeData.answers?.カナ || "なし"}\n`);
  }
  ...
  console.log(`  日時: ${reserveDate} ${reserveTime}`);
  ...
  try {
    ...
    console.log(`  Response: ${JSON.stringify(reservationJson)}`);
  ...
  if (updatedIntake) {
    ...
    const allFieldsPresent = updatedIntake.reserve_id && updatedIntake.reserved_date && updatedIntake.reserved_time;
  ...
  try {
    ...
    const mypageJson = await mypageResponse.json();
    ...
    if (mypageStatus >= 200 && mypageStatus < 300 && mypageJson.ok) {
      ...
      if (reservationInMypage) {
        ...
        console.log(`      予約日時: ${reservationInMypage.reserved_date} ${reservationInMypage.reserved_time}`);
