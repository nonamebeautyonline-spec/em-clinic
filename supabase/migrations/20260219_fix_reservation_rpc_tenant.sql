-- 予約RPCにテナントID対応を追加
-- 問題: RPCが全テナントの予約をカウントしていた → スロット表示(○)なのに確定時にslot_fullエラー
...
BEGIN
  ...
  -- 5. 該当スロットの予約数をカウント（FOR UPDATE でロック、テナントフィルタ付き）
...
BEGIN
  ...
  -- 5. 変更先スロットの予約数をカウント（自分自身は除外、FOR UPDATE でロック、テナントフィルタ付き）
  SELECT COUNT(*) INTO v_booked_count
  FROM (
    SELECT 1 FROM reservations
