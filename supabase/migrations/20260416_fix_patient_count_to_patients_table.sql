-- 患者数カウントをintakeテーブルからpatientsテーブルに統一
-- ダッシュボードの「総患者数」「新規患者数」がintake（問診）ベースだったのをpatients（LINE登録）ベースに修正
-- SSEの todayNewPatients は既にpatientsベースなので、RPC関数側を合わせる
...
BEGIN
  ...
  -- 10. 総患者数（patientsテーブル = LINE友だち追加数）
  ...
  -- 14a. 新規患者数（patientsテーブル = LINE友だち追加数、SSEのtodayNewPatientsと統一）
  ...
  -- 14b. 問診患者ID（KPI「問診後予約率」計算用、intakeベースのまま維持）
