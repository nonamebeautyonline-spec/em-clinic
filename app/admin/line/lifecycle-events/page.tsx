...
// このページで表示する setting_key
const LIFECYCLE_KEYS = ["personal_info_completed", "verification_completed"] as const;

const SETTING_LABELS: Record<string, { title: string; description: string }> = {
  personal_info_completed: {
    title: "個人情報入力完了時",
    description: "患者が個人情報フォームを送信した後に自動実行するアクション",
  },
  verification_completed: {
    title: "電話番号認証完了時",
    description: "患者が電話番号認証（SMS認証）を完了した後に自動実行するアクション",
  },
};

// ── 旧データ → 新ステップ形式へ変換 ────────────────────

function convertLegacyToSteps(sv: EventSetting["setting_value"]): ActionStep[] {
...
export default function LifecycleEventsPage() {
  ...
  // ── カードのアクションサマリ ──
  const renderActionSummary = (sv: EventSetting["setting_value"]) => {
    ...
    <div className="min-h-full bg-gray-50/50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-6">
          ...
          <p className="text-sm text-gray-400 mt-1">患者の登録フロー各ステップで自動実行するアクションを設定</p>
        </div>
      </div>
                ...
                <div key={setting.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-100">
                    <div className="flex items-center justify-between">
                      ...
                      {setting.enabled && (
                        <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] font-bold">有効</span>
                      )}
                    </div>
                  ...
                  <div className="px-6 py-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl text-xs font-medium hover:from-blue-600 hover:to-indigo-700 transition-all shadow-sm flex items-center justify-center gap-1.5"
                      >
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
              <h2 className="font-bold text-gray-900 text-lg">
                {SETTING_LABELS[editingKey]?.title || editingKey} のアクション設定
              </h2>
                    ...
                    {step.type === "send_text" && (
                      <>
                        ...
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-4 py-3 border border-gray-200 rounded-b-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 resize-none"
                        />
                      </>
                    )}
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0">
              <button
                onClick={handleSave}
                disabled={saving}
                className="w-full px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl text-sm font-bold hover:from-blue-600 hover:to-indigo-700 shadow-lg shadow-blue-500/25 transition-all disabled:opacity-40"
              >
