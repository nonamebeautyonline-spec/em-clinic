...
// app/admin/platform/tenants/[tenantId]/page.tsx
...
export default function TenantDetailPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/admin/platform/tenants/${tenantId}/stats`,
        { credentials: "include" },
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setTimeout(() => router.push("/admin/platform/tenants"), 1000);
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : "エラーが発生しました");
    } finally {
      setDeleting(false);
  ...
  const handleAddMember = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/platform/tenants/${tenantId}/members`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const handleRoleChange = async (memberId: string, newRole: string) => {
    try {
      const res = await fetch(
        `/api/admin/platform/tenants/${tenantId}/members/${memberId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
