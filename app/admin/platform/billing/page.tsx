...
function PlansTab({
  ...
  const loadPlans = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/platform/billing/plans?${params}`,
        { credentials: "include" },
      );
  ...
  const savePlan = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/platform/billing/plans/${editModal.tenantId}`,
        {
          method: "PUT",
...
function InvoicesTab({
  ...
  useEffect(() => {
    const loadTenants = async () => {
      try {
        const res = await fetch(
          "/api/admin/platform/tenants?limit=100",
          { credentials: "include" },
        );
        if (res.ok) {
          const data = await res.json();
  ...
  const loadInvoices = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/platform/billing/invoices?${params}`,
        { credentials: "include" },
      );
  ...
  const updateInvoiceStatus = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/platform/billing/invoices/${statusModal.invoiceId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
