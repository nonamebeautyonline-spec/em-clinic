...
export default function MonthlySchedulePage() {
  ...
  // 選択日の予約取得（担当医変更用）
  const reservationsKey = selectedDate ? `/api/admin/reservations?date=${selectedDate}` : null;
              ...
              <div className="flex items-center gap-2">
                {doctors.map((d) => (
                  <button
                    ...
                    style={doctorId === d.doctor_id ? { borderColor: d.color || "#6366f1" } : undefined}
                  >
                    <span
              ...
              {loading ? (
                ...
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-7 mb-2">
                    {WEEKDAYS.map((w, i) => (
                      <div key={w} className={`text-center text-sm font-semibold py-2 ${WEEKDAY_COLORS[i]}`}>
                        {w}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {dayConfigs.map((config, index) => {
                      ...
                      // 時間帯テキストを取得（時間早い順）
                      type TimeEntry = { time: string; sortKey: string };
                        ...
                        if (config.overrides.length > 0) {
                          config.overrides
                            ...
                            .forEach((o) => {
                              ...
                              timeEntries.push({ time: `${sh}-${eh}`, sortKey: o.start_time! });
                            });
                        } else {
                          const rule = weeklyMap.get(d.getDay());
                          if (rule?.enabled && rule.start_time && rule.end_time) {
                            ...
                            timeEntries.push({ time: `${sh}-${eh}`, sortKey: rule.start_time });
                          }
                        }
                        timeEntries.sort((a, b) => a.sortKey.localeCompare(b.sortKey));
                      }
                      ...
                      return (
                        ...
                        >
                          <div className="h-full flex flex-col items-start">
                            <span className={`text-sm font-semibold ${WEEKDAY_COLORS[d.getDay()]} ${!isCurrentMonth ? "text-slate-300" : ""}`}>
                              {d.getDate()}
                            </span>
                            {isCurrentMonth && !isPasteTarget && (
                              <>
                                {config.effectiveStatus === "closed" ? (
                                  <span className="mt-0.5 text-xs text-slate-400">休診</span>
                                  ...
                                  <div className="mt-0.5 space-y-0.5 w-full">
                                    {timeEntries.map((entry, i) => (
                                      ...
                                      >
                                        ...
                                        />
                                        <span className="text-[11px] leading-tight text-slate-700 truncate font-medium">{entry.time}</span>
                                      </div>
                                    ))}
                                  </div>
                                ) : (
                                  <span className="mt-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500" />
                                )}
                              </>
                            )}
                            {isPasteTarget && (
                              <svg className="w-5 h-5 text-blue-600 mt-0.5 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
