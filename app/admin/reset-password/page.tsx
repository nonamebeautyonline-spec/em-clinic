...
const publicFetcher = async (url: string) => {
  ...
  if (!res.ok || !data.ok) {
    throw new Error((data.message || data.error) || "無効なリンクです");
  }
  return data;
};
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 rounded-lg shadow-xl p-8 w-full max-w-md border border-slate-700 text-center">
          ...
          <Link href="/admin/forgot-password" className="text-blue-400 hover:text-blue-300 text-sm">
            パスワードリセットを再度リクエスト
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 rounded-lg shadow-xl p-8 w-full max-w-md border border-slate-700 text-center">
          ...
          <h1 className="text-xl font-bold text-white mb-4">パスワードを設定しました</h1>
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-lg shadow-xl p-8 w-full max-w-md border border-slate-700">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-white mb-2">パスワード設定</h1>
          {userName && <p className="text-slate-400 text-sm">{userName} さん</p>}
        </div>
