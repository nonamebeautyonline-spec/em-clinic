...
export default function FlexSection({ onToast }: Props) {
  ...
  return (
    <div>
      ...
      {/* アコーディオン */}
      <div className={`space-y-2 ${!editing ? "pointer-events-none opacity-60" : ""}`}>
        {TABS.map(({ key, label }) => {
                  ...
                  <div className="border-t border-gray-100">
                    ...
                    {key === "reorder_apply" && (
                      <div className="px-5 py-4 flex items-center justify-between">
                        <div>
                          <p className="text-xs text-gray-500">患者が再処方を申請した際に管理者LINEグループへ通知</p>
                        </div>
                    ...
                    {key === "reorder_paid" && (
                      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 p-5">
                        <div className="lg:col-span-3 space-y-6">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-xs text-gray-500">決済完了時に患者へサンクスメッセージを送信</p>
                            </div>
                          ...
                          {eventConfig.notifyReorderPaid && (
                            <>
                              ...
                              <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                                <div>
                                  <h4 className="text-xs font-bold text-gray-700">ヘッダー設定</h4>
                              ...
                              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                                <div>
                                  <h4 className="text-xs font-bold text-gray-700">表示項目</h4>
                                ...
                                {([
                                  ...
                                  <div key={sk} className="flex items-center justify-between">
                                    <span className="text-sm text-gray-700">{sl}</span>
                                    <Toggle checked={eventConfig[sk]} onChange={(v) => setEventConfig(prev => ({ ...prev, [sk]: v }))} disabled={!editing} />
                                  </div>
                                ))}
                                ...
                                {eventConfig.showShippingInfo && (
                                  <div className="pl-3 space-y-3 border-l-2 border-gray-200">
                                    {([
                              ...
                              <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                                <div>
                                  <h4 className="text-xs font-bold text-gray-700">メッセージ設定</h4>
                    ...
                    {key === "intake_reminder" && (
                      <div className="px-5 py-4">
                        <p className="text-xs text-gray-500 mb-3">問診完了後、指定時間以内に予約がなければ患者にLINEリマインダーを送信</p>
                    ...
                    {/* === Flex系（予約・決済案内・発送） === */}
                    {(key === "reservation" || key === "shipping" || key === "payment") && (
                      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 p-5">
                        <div className="lg:col-span-3 space-y-6">
                          {/* 配色設定 */}
                          <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                            <div>
                              <h4 className="text-xs font-bold text-gray-700">配色設定</h4>
                              <p className="text-xs text-gray-500 mt-0.5">この通知専用の配色</p>
                            </div>
                            {(() => {
                              const tabColors = getColorsForTab(flexConfig, key as FlexTabType);
                              return COLOR_LABELS.map(({ key: ck, label: cl }) => (
                                <div key={ck} className="flex items-center gap-4">
                                  <input type="color" value={tabColors[ck]} onChange={(e) => updateColor(ck, e.target.value)}
                                    className="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer p-0.5" />
                                  <div className="flex-1">
                                    <label className="text-sm text-gray-700 font-medium">{cl}</label>
                                    <input type="text" value={tabColors[ck]}
                                      onChange={(e) => { if (/^#[0-9a-fA-F]{0,6}$/.test(e.target.value)) updateColor(ck, e.target.value); }}
                          ...
                          {key === "reservation" && (
                            <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                              <div>
                                <h4 className="text-xs font-bold text-gray-700">予約通知テキスト</h4>
                              ...
                              <div className="flex gap-2">
                                {([
                                  ...
                                  <button key={sk} onClick={() => setReservationSubTab(sk)}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors ${
                                      reservationSubTab === sk ? "bg-blue-100 text-blue-700" : "bg-white text-gray-500 hover:text-gray-700"
                                    }`}>
                                    {sl}
                                  </button>
                                ))}
                          ...
                          {key === "payment" && (
                            <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                              <div>
                                <h4 className="text-xs font-bold text-gray-700">決済案内テキスト</h4>
                          ...
                          {key === "shipping" && (
                            <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                              <div>
                                <h4 className="text-xs font-bold text-gray-700">発送通知テキスト</h4>
