...
export default function CampaignTab() {
  ...
  const conditionProducts: ProductDef[] = products.map((p) => ({ id: p.id, code: p.code, title: p.title }));
  ...
  const openEdit = (c: Campaign) => {
    ...
    // audience_patient_idsからPatientResult形式にする（表示用にIDのみ）
  ...
  const getStatus = (c: Campaign): { label: string; color: string } => {
    ...
    if (c.max_uses != null && c.used_count >= c.max_uses) return { label: "上限到達", color: "bg-amber-100 text-amber-700" };
    return { label: "開催中", color: "bg-green-100 text-green-700" };
  };

  const categories = [...new Set(products.map((p) => p.category).filter(Boolean))];

  const audienceLabel = (c: Campaign): string => {
    if (c.audience_type === "specific") return `対象: ${c.audience_patient_ids?.length || 0}名`;
    if (c.audience_type === "condition") return "条件指定あり";
    return "全員";
  };
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-slate-600 text-sm">期間限定セール・割引キャンペーンを管理</p>
        </div>
        <button
          onClick={openCreate}
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      {audienceLabel(c)}
                      {c.max_uses != null && (
                        <span className="ml-2">利用 {c.used_count}/{c.max_uses}</span>
                      )}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                      c.is_active ? "border-amber-300 text-amber-700 hover:bg-amber-50" : "border-green-300 text-green-700 hover:bg-green-50"
                    }`}
                  >
                    {c.is_active ? "無効化" : "有効化"}
                  </button>
                  <button onClick={() => openEdit(c)} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50">
                    編集
                  </button>
                  <button onClick={() => handleDelete(c.id)} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-red-300 text-red-700 hover:bg-red-50">
                    削除
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              ...
              {/* 対象商品範囲 */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">対象商品</label>
                <div className="flex gap-2 mb-2">
                  {(["all", "category", "specific"] as const).map((t) => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                        form.target_type === t ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"
                      }`}
                    >
              ...
              {/* 利用上限 */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">利用上限（空=無制限）</label>
                <input
                  type="number"
                  value={form.max_uses}
                  onChange={(e) => setForm({ ...form, max_uses: e.target.value })}
                  placeholder="例: 100（先着100名）"
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                />
              </div>
                ...
                {form.audience_type === "specific" && (
                  <div className="space-y-2">
                    <div className="relative">
                      ...
                      />
                      {searching && (
                        <div className="absolute right-3 top-2.5 text-xs text-slate-400">検索中...</div>
                      )}
