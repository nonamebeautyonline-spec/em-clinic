    ...
    <nav className="fixed top-0 z-50 w-full bg-white/80 backdrop-blur-xl">
      <div className="mx-auto flex h-[56px] max-w-6xl items-center justify-between px-4">
        ...
        <div className="hidden items-center gap-7 md:flex">
          {links.map((l) => <a key={l.href} href={l.href} className="text-[13px] font-medium text-slate-500 transition hover:text-emerald-600">{l.label}</a>)}
          <a href="/line/contact" className="rounded-lg bg-gradient-to-r from-[#06C755] to-emerald-500 px-5 py-2 text-[13px] font-bold text-white shadow-sm shadow-emerald-500/20 transition hover:shadow-md">お問い合わせ</a>
        </div>
      ...
      {open && (
        <div id="mobile-nav-menu" className="border-t border-slate-100 bg-white px-5 pb-5 md:hidden">
          ...
          <a href="/line/contact" className="mt-2 block rounded-lg bg-[#06C755] py-3 text-center text-sm font-bold text-white" onClick={() => setOpen(false)}>お問い合わせ</a>
        </div>
      )}
    </nav>
  );
