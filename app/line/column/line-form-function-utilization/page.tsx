...
const faqItems = [
  ...
  { q: "Googleフォームとの違いは何ですか？", a: "LINEフォームは回答者のLINEアカウントと紐づくため、回答後にセグメント配信や自動フォローが可能です。Googleフォームは汎用性が高いですが、LINE上でのフォローアップ自動化ができません。LINE運用と連動させるならLINEフォームが圧倒的に有利です。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="フォーム機能活用" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <ResultCard before="10問・自由記述多め" after="5問・選択式中心+クーポン特典" metric="フォーム回答率" description="15% → 52%に向上（約3.5倍）" />
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="LINE運用にはLINEフォームが圧倒的に有利">
          Google Formsは汎用ツールとしては優秀ですが、LINE運用との連携を考えるとLINEフォームの方が圧倒的に効率的です。回答データの自動タグ付けとセグメント配信への即時活用が最大のメリットです。
        </Callout>
      </section>
