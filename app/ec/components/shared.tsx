...
/* ──── 汎用UIパーツ（Lオペ for EC — ベージュ/シルバーテーマ） ──── */
...
export function Label({ children }: { children: React.ReactNode }) {
  return <span className="mb-4 inline-block rounded-full bg-stone-50 px-4 py-1.5 text-[11px] font-semibold tracking-[.18em] text-stone-600 uppercase">{children}</span>;
}
...
export function MockWindow({ title, children, className = "" }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`min-w-0 overflow-hidden rounded-xl border border-stone-200/80 bg-white shadow-xl shadow-stone-200/40 ${className}`}>
      <div className="flex items-center gap-2 border-b border-stone-100 bg-stone-50/80 px-4 py-2.5">
        <span className="h-2.5 w-2.5 rounded-full bg-[#FF5F57]" /><span className="h-2.5 w-2.5 rounded-full bg-[#FEBC2E]" /><span className="h-2.5 w-2.5 rounded-full bg-[#28C840]" />
        <span className="ml-3 text-[10px] font-medium text-stone-400">{title}</span>
      </div>
