...
export default function Nav() {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full border-b border-slate-200 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[56px] max-w-6xl items-center justify-between px-4">
        <a href="/ec" className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-bold tracking-tight text-slate-900">Lオペ <span className="text-[11px] font-semibold text-amber-600">for EC</span></span>
        </a>
        <div className="hidden items-center gap-7 md:flex">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-[13px] font-medium text-slate-500 transition hover:text-amber-600">{l.label}</a>
          ))}
        ...
        <button className="md:hidden" onClick={() => setOpen(!open)} aria-label="メニュー" aria-expanded={open} aria-controls="mobile-nav-menu-ec">
          <svg className="h-6 w-6 text-slate-500" fill="none" stroke="currentColor" strokeWidth={1.8} aria-hidden="true">{open ? <path d="M6 6l12 12M6 18L18 6" /> : <path d="M4 7h16M4 12h16M4 17h16" />}</svg>
        </button>
      </div>
      {/* モバイル用クイックナビ */}
      <div className="flex items-center gap-1 overflow-x-auto border-t border-slate-100 px-4 py-1.5 md:hidden">
        {quickLinks.map((l) => (
          <a
            key={l.href}
            href={l.href}
            className={`shrink-0 rounded-full px-3 py-1.5 text-[11px] font-semibold transition ${
              l.accent
                ...
                : "bg-slate-100 text-slate-500 hover:bg-amber-500/10 hover:text-amber-600"
            }`}
          >
            {l.label}
          </a>
      ...
      {open && (
        <div id="mobile-nav-menu-ec" className="border-t border-slate-200 bg-white px-5 pb-5 md:hidden">
          {links.map((l) => <a key={l.href} href={l.href} className="block py-3 text-sm text-slate-500" onClick={() => setOpen(false)}>{l.label}</a>)}
