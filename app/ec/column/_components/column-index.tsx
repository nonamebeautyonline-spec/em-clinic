...
const categoryStyles: Record<string, { pill: string }> = {
  "EC×LINE活用入門":  { pill: "bg-amber-50 text-amber-700 ring-amber-200/50" },
  "カゴ落ち対策":      { pill: "bg-rose-50 text-rose-600 ring-rose-200/50" },
  "配信・リピート促進": { pill: "bg-violet-50 text-violet-600 ring-violet-200/50" },
  "顧客管理・CRM":     { pill: "bg-stone-50 text-stone-600 ring-stone-200/50" },
  "発送管理・物流":     { pill: "bg-sky-50 text-sky-600 ring-sky-200/50" },
  "業態別活用事例":     { pill: "bg-orange-50 text-orange-600 ring-orange-200/50" },
  "分析・改善":         { pill: "bg-teal-50 text-teal-600 ring-teal-200/50" },
...
/* ─── 注目記事（3記事） ─── */
const featuredSlugs = [
  "line-cart-abandonment-recovery-guide",
  "ec-line-crm-strategy",
  "ec-line-sales-20-percent-up-case-study",
];

/* ─── 人気記事 ─── */
const popularSlugs = [
  "line-cart-abandonment-recovery-guide",
  "ec-segment-delivery-purchase-data",
  "ec-shipping-notification-line-automation",
  "ec-line-block-rate-reduction",
  "stripe-line-payment-completion-rate",
];

/* ─── 人気タグ ─── */
const popularTags = [
  "カゴ落ち対策", "セグメント配信", "リピート率", "CRM",
  "発送通知", "D2C", "クーポン", "RFM分析",
  "Stripe連携", "LINE公式アカウント",
];
...
export default function ColumnIndex() {
  ...
  return (
    <div className="min-h-screen bg-amber-100/60 text-gray-800">
      {/* ヘッダー */}
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {categories.map((cat) => {
              ...
              return (
                <a
                  key={cat.slug}
                  href={`/ec/column/category/${cat.slug}`}
                  className="group rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 transition hover:border-amber-200 hover:bg-amber-50"
                >
      ...
      <div className="border-b border-gray-200 bg-gradient-to-b from-white to-gray-50 px-6 pb-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-4 flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-stone-700 to-amber-700 px-3 py-1 text-[12px] font-bold text-white shadow-sm">
            ...
            {featuredSlugs.map((slug) => {
              ...
              return (
                <a
                  ...
                  className="group relative overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-gray-200/60 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-100/50 hover:ring-amber-300/80"
                >
            ...
            return (
              <button
                ...
                className={`shrink-0 rounded-full px-5 py-2 text-[14px] font-medium ring-1 transition ${
                  isActive
                    ? "bg-gradient-to-r from-stone-700 to-amber-700 text-white ring-amber-700 shadow-sm"
                    : "bg-white text-gray-500 ring-gray-200 hover:bg-gray-50 hover:text-gray-700"
                }`}
              >
                {cat}
                <span className={`ml-1.5 text-[12px] ${isActive ? "text-amber-200" : "text-gray-300"}`}>{count}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex gap-10">
          ...
          <main className="min-w-0 flex-1">
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((a) => (
                <a
                  key={a.slug}
                  href={`/ec/column/${a.slug}`}
                  className="group overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200/60 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:ring-amber-200/80"
                >
                  {/* サムネイル */}
                  <div className="overflow-hidden">
                    <div className="transition-transform duration-300 group-hover:scale-[1.03]">
                  ...
                  <div className="px-5 py-4">
                    ...
                    {/* タイトル */}
                    <h3 className="mt-2.5 text-[15px] font-bold leading-snug text-gray-900 line-clamp-2 group-hover:text-amber-700 transition-colors">
                      {a.title}
                    </h3>
            ...
            {/* CTA */}
            <div className="mt-14 overflow-hidden rounded-2xl bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 p-10 text-center ring-1 ring-amber-100 md:p-14">
              <p className="text-[12px] font-bold tracking-widest text-amber-600 uppercase">Lオペ for EC</p>
              <h2 className="mt-3 text-[22px] font-bold text-gray-800 md:text-[26px]">ECのLINE運用をもっと効率的に</h2>
              <p className="mx-auto mt-3 max-w-md text-[15px] leading-relaxed text-gray-500">
                カゴ落ち対策・発送通知・CRM・分析をオールインワンで。<br className="hidden sm:block" />まずはお気軽にお問い合わせください。
              </p>
          ...
          <aside className="hidden w-72 shrink-0 lg:block">
            <div className="sticky top-20 space-y-8">
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200/60">
                ...
                <div className="mt-4 space-y-0 divide-y divide-gray-100">
                  {popular.map((a, i) => (
                    ...
                    >
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gradient-to-r from-stone-700 to-amber-700 text-[11px] font-bold text-white">
                        {i + 1}
                      </span>
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <a
                        key={cat.slug}
                        href={`/ec/column/category/${cat.slug}`}
                        className="flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-[14px] text-gray-600 hover:bg-amber-50 hover:text-amber-700 transition"
                      >
                        {cat.label}
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200/60">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {popularTags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-block rounded-full bg-gray-50 px-3 py-1.5 text-[12px] text-gray-600 ring-1 ring-gray-200/60 transition hover:bg-amber-50 hover:text-amber-700 hover:ring-amber-200/50 cursor-default"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              {/* サイドバーCTA */}
              <div className="rounded-2xl bg-gradient-to-br from-stone-50 to-amber-50 p-6 text-center ring-1 ring-amber-100">
                <p className="text-[14px] font-bold text-gray-800">EC×LINE運用のご相談はこちら</p>
