...
const categoryStyles: Record<string, string> = {
  "EC×LINE活用入門":  "bg-amber-50 text-amber-700 ring-amber-200/50",
  "カゴ落ち対策":      "bg-rose-50 text-rose-600 ring-rose-200/50",
  "配信・リピート促進": "bg-violet-50 text-violet-600 ring-violet-200/50",
  "顧客管理・CRM":     "bg-stone-50 text-stone-600 ring-stone-200/50",
  "発送管理・物流":     "bg-sky-50 text-sky-600 ring-sky-200/50",
  "業態別活用事例":     "bg-orange-50 text-orange-600 ring-orange-200/50",
  "分析・改善":         "bg-teal-50 text-teal-600 ring-teal-200/50",
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${cat.label}の記事一覧 | コラム | Lオペ for EC`;
  const description = cat.description;
  const url = `${SITE_URL}/ec/column/category/${cat.slug}`;
...
export default async function CategoryPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "CollectionPage",
    name: `${cat.label}の記事一覧 — Lオペ for EC コラム`,
    description: cat.description,
    url: `${SITE_URL}/ec/column/category/${cat.slug}`,
        ...
        <header className="sticky top-0 z-30 border-b border-gray-200/60 bg-white/95 backdrop-blur-md">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3.5">
            ...
            <div className="flex items-center gap-6">
              <Link href="/ec/about" className="hidden text-[14px] text-gray-500 hover:text-amber-700 transition md:block">
                Lオペ for ECとは
              </Link>
              <Link href="/ec/features" className="hidden text-[14px] text-gray-500 hover:text-amber-700 transition md:block">
                機能一覧
              </Link>
              <Link href="/ec/column" className="hidden text-[14px] font-medium text-amber-700 md:block">
                コラム
              </Link>
              <Link
                href="/ec/contact"
        ...
        <div className="border-b border-gray-200 bg-white">
          <nav aria-label="パンくずリスト" className="mx-auto max-w-7xl px-6 py-3">
            <ol className="flex items-center gap-2 text-[13px] text-gray-400 list-none m-0 p-0">
              <li>
                <Link href="/ec" className="hover:text-amber-700 transition">
                  トップ
                </Link>
              </li>
              ...
              <li>
                <Link href="/ec/column" className="hover:text-amber-700 transition">
                  コラム
                </Link>
              </li>
              <li aria-hidden="true" className="text-gray-300">
          ...
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: cat.label, item: `https://l-ope.jp/ec/column/category/${cat.slug}` },
            ],
          }) }} />
        </div>
          ...
          <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2.5 pb-5">
            ...
            {categories.map((c) => {
              ...
              return (
                <Link
                  key={c.slug}
                  href={`/ec/column/category/${c.slug}`}
                ...
                >
                  {c.label}
                  <span className={`ml-1.5 text-[12px] ${isActive ? "text-amber-200" : "text-gray-300"}`}>
                    {count}
                  </span>
                </Link>
              );
        ...
        <div className="mx-auto max-w-7xl px-6 py-10">
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {articleList.map((a) => (
              <Link
                ...
                className="group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200/60 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-100/50 hover:ring-amber-200/80"
              >
                {/* サムネイル */}
                ...
                <div className="px-6 py-5">
                  ...
                  <span className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-semibold text-amber-700 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                    続きを読む
          ...
          <section className="mt-12 border-t border-gray-200 pt-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                href="/ec/column"
                className="px-4 py-2 bg-amber-50 rounded-lg text-sm text-amber-700 hover:bg-amber-100 transition"
              >
                すべての記事
              </Link>
            </div>
          ...
          <div className="mt-14 overflow-hidden rounded-2xl bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 p-10 text-center ring-1 ring-amber-100 md:p-14">
            <p className="text-[12px] font-bold tracking-widest text-amber-600 uppercase">
              Lオペ for EC
            </p>
        ...
        <footer className="border-t border-gray-200 bg-white py-10">
          <div className="mx-auto max-w-7xl px-6">
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
              ...
              <div className="flex items-center gap-6">
                <Link href="/ec" className="text-[13px] text-gray-400 hover:text-amber-700 transition">
                  トップ
                </Link>
                <Link href="/ec/about" className="text-[13px] text-gray-400 hover:text-amber-700 transition">
                  Lオペ for ECとは
                </Link>
                <Link href="/ec/features" className="text-[13px] text-gray-400 hover:text-amber-700 transition">
                  機能一覧
                </Link>
                <Link href="/ec/column" className="text-[13px] text-amber-700 font-medium">
                  コラム
                </Link>
              </div>
            </div>
