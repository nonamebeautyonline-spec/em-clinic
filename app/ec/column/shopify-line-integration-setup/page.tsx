...
const faqItems = [
  { q: "Shopify×LINE連携に技術的な知識は必要ですか？", a: "Lオペ for ECを使えば、Shopifyの管理画面からワンクリックでLINE連携が可能です。Webhook設定やAPI接続はツール側で自動処理されるため、プログラミングの知識は不要です。" },
  { q: "連携後に自動配信できるメッセージの種類は？", a: "カゴ落ちリマインド・注文確認・発送通知・配達完了通知・レビュー依頼・再入荷通知など、購入フロー全体のメッセージを自動配信できます。配信タイミングや文面はカスタマイズ可能です。" },
  { q: "Shopify以外のECカートとも連携できますか？", a: "はい、BASE・STORES・EC-CUBE・カラーミーショップなど主要ECカートに対応しています。各カートのAPI/Webhookを通じて注文・カート・発送データを自動取得します。" },
];
