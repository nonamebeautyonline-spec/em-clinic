  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>第二に、<strong>法的保護</strong>の観点です。万一の医療訴訟において、カルテの記載内容は最も重要な証拠資料となります。オンライン診療で対面を勧めたにもかかわらず患者が拒否した場合など、診療経過を詳細に記録しておくことが医師の法的保護につながります。<Link href="/clinic/column/online-clinic-consent-form-guide" className="text-sky-600 underline hover:text-sky-800">同意書の設計</Link>と合わせて整備することで、法的リスクをさらに低減できます。</p>
      ...
      <section>
        ...
        <p>特に注意すべきは<strong>身体所見の記載</strong>です。対面診療では「胸部聴診：清」などと記載しますが、オンライン診療では聴診を実施できません。「オンライン診療のため聴診は未実施」と明記することで、実施していない検査を実施したかのような誤解を防ぎます。<Link href="/clinic/column/online-clinic-first-visit-revisit-rules" className="text-sky-600 underline hover:text-sky-800">初診・再診で異なる診察要件</Link>も記載内容に影響するため、併せて確認しましょう。</p>
      ...
      <section>
        ...
        <p>対面診療では、診察室で患者の話を聞きながらカルテに入力するワークフローが自然ですが、オンライン診療では画面を見ながらの入力が難しいケースがあります。<strong>事前問診で基本情報を収集済み</strong>であれば、ビデオ通話中は患者の表情や訴えに集中でき、診察の質が向上します。<Link href="/clinic/column/online-questionnaire-guide" className="text-sky-600 underline hover:text-sky-800">オンライン問診の設計と活用方法</Link>も参考にしてください。</p>
