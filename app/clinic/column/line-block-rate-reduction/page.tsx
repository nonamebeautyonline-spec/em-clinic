  ...
  return (
    ...
    <ArticleLayout slug={self.slug} breadcrumbLabel="ブロック率対策" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>ブロック理由の第2位は「自分に関係のない情報が送られてくる」です。この問題を解決するのが<strong>セグメント配信</strong>です。セグメント配信の具体的な設計手法やリピート率向上の事例は<Link href="/clinic/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">LINEセグメント配信でリピート率を向上</Link>で詳しく解説しています。</p>
      ...
      <section>
        ...
        <p>ある皮膚科クリニック（友だち数3,200人）の事例をご紹介します。他のクリニックの成功事例は<Link href="/clinic/column/clinic-line-case-studies" className="text-sky-600 underline hover:text-sky-800">クリニックのLINE公式アカウント活用事例5選</Link>でも紹介しています。LINE公式アカウント開設当初は一斉配信を週2回行っており、ブロック率は<strong>5%</strong>に達していました。</p>
      ...
      <section>
        ...
        <p className="mt-4">Lオペ for CLINICなら、<Link href="/clinic/features#メッセージ配信" className="text-sky-600 underline hover:text-sky-800">セグメント配信</Link>・配信スケジュール管理・<Link href="/clinic/features#分析・レポート" className="text-sky-600 underline hover:text-sky-800">ブロック率分析ダッシュボード</Link>までオールインワンで提供。患者に嫌われない配信運用を、すぐに始められます。ブロック率を含むクリニック経営のKPI管理については<Link href="/clinic/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">KPI7選</Link>も参考にしてください。</p>
      </section>
