...
const faqItems = [
  { q: "オンライン診療の適切な実施に関する指針を始めるために必要な準備は何ですか？", a: "厚生労働省のオンライン診療ガイドラインに基づく届出、ビデオ通話システムの導入、オンライン決済の設定が必要です。Lオペ for CLINICならLINEビデオ通話・電話音声通話でのオンライン診療に対応しており、別途システム導入が不要です。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>
          リスクを回避するためには、<strong>指針の要件を一つひとつシステムに落とし込み、運用フローに組み込む</strong>ことが最善策です。チェックリストの定期確認、スタッフ研修、外部専門家によるコンプライアンス監査を組み合わせることで、持続的に指針に準拠した運用を維持できます。開業時に必要な法的手続きの詳細は<Link href="/clinic/column/online-clinic-legal-setup-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療の開設届・法的手続きガイド</Link>もご参照ください。
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          オンライン診療の導入を検討しているクリニック、または既に運用しているクリニックは、指針の要件を定期的に見直し、自院の運用がルールに適合しているかを確認することが重要です。オンライン診療の全体像は<Link href="/clinic/column/online-clinic-complete-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療完全ガイド</Link>、法規制の体系的な解説は<Link href="/clinic/column/online-clinic-regulations" className="text-sky-600 underline hover:text-sky-800">オンライン診療の法規制と薬機法</Link>、開業の法的手続きは<Link href="/clinic/column/online-clinic-legal-setup-guide" className="text-sky-600 underline hover:text-sky-800">開設届・法的手続きガイド</Link>もあわせてご覧ください。お問い合わせは<Link href="/clinic/contact" className="text-sky-600 underline hover:text-sky-800">こちら</Link>から。
        </p>
      </section>
