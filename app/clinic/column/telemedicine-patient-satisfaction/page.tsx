  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="活用事例" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>オンライン診療を利用した患者の満足度調査では、全体の<strong>76%が「満足」または「やや満足」</strong>と回答しています（調査結果の一例では）。しかし、「非常に満足」と回答した割合は28%にとどまっており、改善の余地は大きいと言えます。</p>
      ...
      <section>
        ...
        <p>このように予約から診察開始までのすべてのステップをLINE上で完結させることで、患者はストレスなく診察に臨むことができます。予約最適化の詳細については<Link href="/clinic/column/online-clinic-complete-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療完全ガイド</Link>も合わせてご確認ください。</p>
      </section>
      ...
      <section>
        ...
        <p>NPS調査の詳しい実施方法については<Link href="/clinic/column/clinic-nps-survey" className="text-sky-600 underline hover:text-sky-800">クリニックのNPS調査実践ガイド</Link>で詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>Lオペ for CLINICは、オンライン診療の予約・問診・配送通知・フォローアップ・NPS調査までをLINE上でワンストップ管理できるクリニック専用プラットフォームです。患者体験の設計から満足度の可視化、口コミ獲得までの一連の仕組みを、手間なく構築できます。オンライン診療の問診設計については<Link href="/clinic/column/online-clinic-questionnaire-design" className="text-sky-600 underline hover:text-sky-800">オンライン問診設計ガイド</Link>、配送フローの最適化については<Link href="/clinic/column/online-clinic-prescription-delivery" className="text-sky-600 underline hover:text-sky-800">医薬品配送ガイド</Link>もあわせてご確認ください。</p>
      </section>
