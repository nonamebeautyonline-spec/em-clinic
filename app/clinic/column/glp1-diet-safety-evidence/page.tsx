...
const faqItems = [
  { q: "GLP-1ダイエットは危険？はオンライン診療で処方できますか？", a: "多くの場合、オンライン診療での処方が可能です。ただし初診では処方日数に制限がある場合があります。再診であれば対面診療と同等の処方が可能です。詳しくは各薬剤の処方ルールをご確認ください。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="医薬品解説" keyPoints={keyPoints} toc={[]}>
      ...
      <section>
        ...
        <ul className="list-disc pl-6 space-y-1">
          <li>
            <Link href="/clinic/column/glp1-medication-comparison" className="text-sky-600 underline hover:text-sky-800">
              GLP-1受容体作動薬の比較ガイド — リベルサス・オゼンピック・マンジャロの仕組みと選び方
            </Link>
          </li>
          {articles.find((a) => a.slug === "rybelsus-side-effects-guide") && (
            <li>
              <Link href="/clinic/column/rybelsus-side-effects-guide" className="text-sky-600 underline hover:text-sky-800">
          ...
          {articles.find((a) => a.slug === "rybelsus-effective-dosing") && (
            <li>
              <Link href="/clinic/column/rybelsus-effective-dosing" className="text-sky-600 underline hover:text-sky-800">
