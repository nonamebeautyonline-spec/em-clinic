  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="経営戦略" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>3月のピーク時は12月の20倍の需要が発生します。このカーブを理解した上で、<strong>12〜1月に先行集患を仕掛け、2月に初期療法を開始させ、3〜4月のピーク需要を確実に取り込む</strong>というスケジュールを組むことが重要です。オンライン診療の開業準備については<Link href="/clinic/column/online-clinic-complete-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療の完全ガイド</Link>も参照してください。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="自費vs保険の判断基準">
          花粉症のオンライン診療を自費で行うか保険で行うかは、クリニックの戦略次第です。<strong>自費診療</strong>は価格設定の自由度が高く利益率も高い一方、保険適用を求める患者層を取りこぼすリスクがあります。<strong>保険診療</strong>は患者の金銭的ハードルが低く集患しやすい反面、レセプト業務やオンライン資格確認の対応が必要です。Dr1人運営のオンラインクリニックでは自費診療に特化し、「待ち時間ゼロ・LINE完結・最短翌日配送」の付加価値で価格を正当化する戦略が主流です。保険と自費の使い分けについては<Link href="/clinic/column/self-pay-pricing-guide" className="text-sky-600 underline hover:text-sky-800">自費診療の価格設定ガイド</Link>も参考になります。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>このパターン化により、オンライン問診の回答内容からほぼ自動的に処方候補が絞られ、医師は最終確認のみで処方を確定できます。オンライン問診の設計方法は<Link href="/clinic/column/online-questionnaire-guide" className="text-sky-600 underline hover:text-sky-800">オンライン問診の設計ガイド</Link>で解説しています。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="info" title="初期療法のLINE配信タイミング">
          関東圏の場合、スギ花粉の飛散開始は例年2月中旬です。初期療法を2月上旬に開始するためには、<strong>1月中旬〜下旬にLINE配信で案内を届ける</strong>必要があります。セグメント配信機能で前年の花粉症患者にだけ配信すれば、無駄なメッセージコストを抑えつつ高い反応率を得られます。タグ管理で「花粉症」タグが付いた患者を自動抽出し、テンプレートメッセージで配信を効率化しましょう。セグメント配信の活用については<Link href="/clinic/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">セグメント配信ガイド</Link>を参照してください。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>ここで重要なのは、花粉症患者に無差別に広告を送るのではなく、<strong>タグ管理とセグメント配信で適切なターゲティングを行う</strong>ことです。例えば、花粉症で受診した30代男性には「AGAの早期治療」の情報を、30代女性には「低用量ピルのオンライン処方」の情報を、それぞれ最適なタイミングで配信します。このセグメント設計がブロック率を下げつつ、コンバージョン率を最大化する鍵です。詳しい展開方法は<Link href="/clinic/column/self-pay-clinic-revenue-triple" className="text-sky-600 underline hover:text-sky-800">自費クリニック売上3倍化の戦略</Link>で解説しています。</p>

        <Callout type="point" title="花粉症患者は最高の「入口」">
          花粉症患者の特徴は、「健康に問題を感じているが、重病ではない」「毎年の受診行動がある」「オンラインで済ませたいニーズがある」の3点です。この特性は、自費オンライン診療の理想的な顧客プロファイルと完全に一致します。花粉症を「入口」として獲得した患者をCRM（患者管理機能）で適切に管理し、ライフステージや性別に応じた他の診療を提案することで、1人あたりのLTV（生涯顧客価値）を大幅に高められます。患者LTVの最大化については<Link href="/clinic/column/clinic-patient-ltv" className="text-sky-600 underline hover:text-sky-800">クリニック患者LTV最大化ガイド</Link>も参考にしてください。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>この先行集患戦略のポイントは、<strong>CPCが安い12〜1月に患者リスト（LINE友だち）を獲得し、2月以降はLINE配信（コスト0）で来院を促す</strong>という構造です。広告費は新規のLINE登録獲得に集中し、一度LINE登録した患者への再アプローチはすべてLINE上で完結させます。友だち追加施策については<Link href="/clinic/column/clinic-line-friends-growth" className="text-sky-600 underline hover:text-sky-800">LINE友だち増加ガイド</Link>も参考になります。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="success" title="ストック型ビジネスの威力">
          花粉症オンライン診療の成長カーブは「直線」ではなく「複利」です。1年目に獲得した花粉症患者が2年目にリピートし、その患者に他の自費診療を提案し、さらにその患者が口コミで新しい患者を連れてくる。LINEという継続的な接点があるからこそ、この成長サイクルが回り続けます。3年目には広告費を増やさなくても、リピーターと紹介だけで十分な売上が立つ構造が完成します。固定費の最適化については<Link href="/clinic/column/clinic-fixed-cost-optimization" className="text-sky-600 underline hover:text-sky-800">クリニック固定費最適化ガイド</Link>で詳しく解説しています。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>花粉症オンライン診療の開業を検討されている方は、ぜひ<Link href="/clinic/contact" className="text-sky-600 underline hover:text-sky-800">Lオペ for CLINICの無料相談</Link>をご利用ください。季節別の集患戦略設計から、Lオペの初期設定・配信シナリオの構築まで、専門スタッフが伴走いたします。</p>

        <p>関連記事: <Link href="/clinic/column/hay-fever-online-clinic-lope" className="text-sky-600 underline hover:text-sky-800">花粉症オンライン診療ガイド</Link> / <Link href="/clinic/column/online-clinic-complete-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療の完全ガイド</Link> / <Link href="/clinic/column/self-pay-pricing-guide" className="text-sky-600 underline hover:text-sky-800">自費診療の価格設定ガイド</Link> / <Link href="/clinic/column/one-room-clinic-simulation" className="text-sky-600 underline hover:text-sky-800">ワンルームクリニック収益シミュレーション</Link> / <Link href="/clinic/column/clinic-fixed-cost-optimization" className="text-sky-600 underline hover:text-sky-800">クリニック固定費最適化ガイド</Link></p>
      </section>
