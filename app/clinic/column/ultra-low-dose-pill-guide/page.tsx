...
const faqItems = [
  { q: "超低用量ピル（LEP製剤）の処方ガイドはオンライン診療で処方できますか？", a: "多くの場合、オンライン診療での処方が可能です。ただし初診では処方日数に制限がある場合があります。再診であれば対面診療と同等の処方が可能です。詳しくは各薬剤の処方ルールをご確認ください。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="医薬品解説" keyPoints={keyPoints} toc={[]}>
      ...
      <section>
        ...
        <p>特に機能性月経困難症は「検査で異常がないのに痛い」ケース。鎮痛剤（NSAIDs）で対応しきれない場合にLEP製剤へステップアップするのが一般的な流れです。<strong>若年患者でも保険が使える</strong>という点は、患者さんにとって大きな安心材料になります。子宮内膜症が原因の器質性月経困難症には<Link href="/clinic/column/dienogest-endometriosis-guide" className="text-sky-600 underline hover:text-sky-800">ジエノゲストの処方ガイド</Link>もあわせてご確認ください。</p>
      ...
      <section>
        ...
        <Callout type="point" title="関連記事もあわせてどうぞ">
          低用量ピル全般の種類比較は<Link href="/clinic/column/pill-types-comparison" className="text-emerald-700 underline">「低用量ピルの種類と選び方」</Link>、初めてピルを飲む方向けの服用ガイドは<Link href="/clinic/column/low-dose-pill-beginners-guide" className="text-emerald-700 underline">「低用量ピルの飲み方完全ガイド」</Link>、月経移動に使う中用量ピルについては<Link href="/clinic/column/medium-dose-pill-guide" className="text-emerald-700 underline">「中用量ピルの処方ガイド」</Link>をご覧ください。
        </Callout>
      </section>
