  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="友だち集め" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>さらに、友だち数が増えるとセグメント配信の精度も向上します。母数が多いほどセグメントごとのサンプル数が確保でき、配信効果の検証・改善サイクルが回しやすくなるためです。セグメント配信の具体的な手法は<Link href="/clinic/column/segment-delivery-repeat" className="text-emerald-700 underline">LINEセグメント配信でリピート率を向上させる方法</Link>で詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="success" title="POPデザインのポイント">
          「友だち追加で何ができるか」を明確に伝えましょう。「LINEで予約確認・変更ができます」「診察結果をLINEで受け取れます」など、患者にとっての具体的なメリットを打ち出すことが重要です。リッチメニューの設計次第で友だち追加後の体験が大きく変わるため、<Link href="/clinic/column/rich-menu-design" className="text-emerald-700 underline">リッチメニュー設計5つのポイント</Link>も合わせて確認しておきましょう。
        </Callout>
      </section>
      ...
      <section>
        ...
        <Callout type="info" title="SNS流入は潜在患者が中心">
          LINEに登録してもらった後は、ステップ配信でクリニックの魅力や症例実績を段階的に伝え、初回来院につなげるナーチャリングが重要です。SNSからの集患戦略は<Link href="/clinic/column/clinic-tiktok-youtube-strategy" className="text-sky-600 underline hover:text-sky-800">TikTok・YouTube活用ガイド</Link>も参考にしてください。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>KPIは週次で確認し、目標に届いていない施策があれば原因を分析して改善を繰り返しましょう。友だち追加数はLINE公式アカウントの管理画面やLオペのダッシュボードでリアルタイムに確認できます。KPIの設計方法については<Link href="/clinic/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">経営KPI7選</Link>で詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>LINE友だち数はクリニックの売上を左右する最重要指標の一つです。院内施策（POP・声かけ・特典）を軸に、Web・SNS・オフラインの導線を組み合わせることで、<strong>月100人以上のペースで友だちを増やす</strong>ことが可能です。友だちを増やした後はブロックされないことも重要です。<Link href="/clinic/column/line-block-rate-reduction" className="text-emerald-700 underline">ブロック率を下げる5つの鉄則</Link>も併せてご覧ください。</p>
