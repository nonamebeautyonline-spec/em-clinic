...
const faqItems = [
  { q: "自費クリニックの患者データ分析で売上を伸ばす最も効果的な方法は？", a: "既存患者へのセグメント配信が最も即効性があります。来院履歴・診療内容に基づいて、関連する自費メニューをLINEで個別提案することで、押し売り感なく自費転換率を高められます。導入クリニックでは自費率が15%→35%に向上した事例もあります。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="経営戦略" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>保険診療と異なり、自費クリニックの経営は<strong>マーケティング・サービス・価格設定のすべてが自院の裁量</strong>に委ねられています。この自由度は、データに基づいて最適化すれば大きなリターンを生みますが、感覚に頼って判断すると容易に経営が迷走します。価格設定の最適化については<Link href="/clinic/column/self-pay-clinic-pricing-strategy" className="text-sky-600 underline hover:text-sky-800">価格設定戦略ガイド</Link>も参考にしてください。</p>
      ...
      <section>
        ...
        <p>計測にあたっては、初診問診票に「来院きっかけ」の選択肢を設け、チャネル別の新患数を毎月集計します。主要チャネルとして、<strong>Google検索（自然検索＋広告）、Instagram、LINE友だち追加、口コミ紹介、ポータルサイト</strong>の5つを最低限追跡しましょう。各媒体のROI比較については<Link href="/clinic/column/self-pay-clinic-ad-roi" className="text-sky-600 underline hover:text-sky-800">広告費ROI最適化ガイド</Link>で詳しく解説しています。</p>
      ...
      <section>
        ...
        <p>LTV向上の具体的な施策については<Link href="/clinic/column/self-pay-clinic-ltv-maximize" className="text-sky-600 underline hover:text-sky-800">自費クリニックのLTV最大化ガイド</Link>で体系的に解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>NPS調査の詳しい設計方法については<Link href="/clinic/column/clinic-nps-survey" className="text-sky-600 underline hover:text-sky-800">クリニックのNPS調査設計ガイド</Link>もあわせてご覧ください。NPSは「患者の声」を経営指標として活用する最も洗練された手法です。</p>
      </section>
      ...
      <section>
        ...
        <p>KPIダッシュボードの具体的な構築方法は<Link href="/clinic/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">クリニックKPIダッシュボード構築ガイド</Link>でさらに詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>データに基づく経営判断は、自費クリニックの成長を加速させる最も確実な方法です。KPIダッシュボードの構築については<Link href="/clinic/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">クリニックKPIダッシュボード構築ガイド</Link>、NPS調査の設計については<Link href="/clinic/column/clinic-nps-survey" className="text-sky-600 underline hover:text-sky-800">クリニックのNPS調査設計ガイド</Link>もあわせてご覧ください。「感覚経営」から「データ経営」へ。この転換が、クリニックの持続的な成長を支える基盤になります。</p>
      </section>
