...
const faqItems = [
  { q: "電子処方箋の導入ガイドを始めるために必要な準備は何ですか？", a: "厚生労働省のオンライン診療ガイドラインに基づく届出、ビデオ通話システムの導入、オンライン決済の設定が必要です。Lオペ for CLINICならLINEビデオ通話・電話音声通話でのオンライン診療に対応しており、別途システム導入が不要です。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>
          自費診療のオンラインクリニックでは、<strong>院内処方（クリニックが薬を直接配送）</strong>を採用しているケースが多いです。この場合、電子処方箋は使わず、クリニック内で在庫から出荷して配送するフローになります。自費オンライン診療の処方に関する法的ルールは<Link href="/clinic/column/self-pay-online-clinic-rules" className="text-sky-600 underline hover:text-sky-800">自費オンライン診療のルールガイド</Link>を参照してください。一方、薬局と連携して調剤・配送を行うモデルであれば、電子処方箋の活用が業務効率化に大きく貢献します。
        </p>
        <p>
