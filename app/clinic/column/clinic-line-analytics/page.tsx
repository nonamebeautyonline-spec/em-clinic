...
export default function Page() {
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <p className="text-[15px] leading-relaxed text-gray-700 font-medium bg-blue-50 rounded-xl p-5 border border-blue-100">LINE配信を「送りっぱなし」にしていませんか？ 効果測定なしの配信は、患者のブロックを招くだけでなく、通数課金の無駄遣いにもつながります。本記事では、開封率・CV率・ROIの計測から<strong>ABテストによる継続的な改善</strong>まで、データドリブンなLINE運用の実践方法を解説します。ブロック率を下げるコツは<Link href="/clinic/column/line-block-rate-reduction" className="text-sky-600 underline hover:text-sky-800">ブロック率を下げる鉄則</Link>で詳しく解説しています。</p>
      ...
      <section>
        ...
        <p>これらの指標を一元管理する方法は<Link href="/clinic/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">LINEダッシュボードで見るべきKPI7選</Link>で詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>LINE導入のROI算出方法をさらに詳しく知りたい方は<Link href="/clinic/column/clinic-line-roi" className="text-sky-600 underline hover:text-sky-800">LINE公式アカウント導入ROIの計算方法</Link>もご覧ください。</p>
      </section>
      ...
      <section>
        ...
        <p>セグメント配信の設計方法と合わせて活用することで、より精度の高い改善が可能です。詳しくは<Link href="/clinic/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">セグメント配信でリピート率を向上させる方法</Link>をご覧ください。</p>
      </section>
