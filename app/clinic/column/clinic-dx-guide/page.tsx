  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="DXガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <Callout type="point" title="成功するDXの3原則">
          <ol className="list-decimal pl-5 space-y-1 mt-1">
            <li><strong>クリニック専用のオールインワンツール</strong>を選ぶ（汎用ツールとの違いは<Link href="/clinic/column/lstep-vs-clinic-tool" className="text-emerald-700 underline">Lステップ・Liny vs クリニック専用ツール比較</Link>を参照）</li>
      ...
      <section>
        ...
        <p>クリニックDXは、LINE公式アカウントという「患者がすでに使っているチャネル」から始めるのが最も効果的です。紙→デジタル、電話→LINE、手作業→自動化を段階的に進めることで、<strong>スタッフの業務負担を削減しながら患者満足度を向上</strong>させることができます。リッチメニューの設計ポイントは<Link href="/clinic/column/rich-menu-design" className="text-emerald-700 underline">リッチメニュー設計5つのポイント</Link>で詳しく解説しています。</p>
