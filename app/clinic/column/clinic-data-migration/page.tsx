...
const faqItems = [
  { q: "クリニックのデータ移行ガイドの導入にどのくらいの期間がかかりますか？", a: "基本的な設定は1〜2週間で完了します。LINE公式アカウントの開設からリッチメニュー設計・自動メッセージ設定まで、Lオペ for CLINICなら初期設定サポート付きで最短2週間で運用開始できます。" },
  { q: "クリニックのデータ移行ガイドでスタッフの負荷は増えませんか？", a: "むしろ減ります。電話対応・手動での予約管理・問診確認などの定型業務を自動化することで、スタッフの作業時間を月40時間以上削減できた事例もあります。導入初月はサポートを受けながら進めれば、2ヶ月目以降はスムーズに運用できます。" },
    ...
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <Callout type="warning" title="よくある失敗：計画不足による手戻り">
          「とりあえず移行してみよう」で始めると、データの不整合や欠損が本番で発覚し、大幅な手戻りが発生します。特に<Link href="/clinic/column/electronic-medical-record-guide" className="text-sky-600 underline hover:text-sky-800">電子カルテ</Link>のデータは構造が複雑なため、事前のマッピング設計が不可欠です。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>全データを一括で移行するのではなく、段階的に移行することでリスクを最小化できます。<Link href="/clinic/column/clinic-crm-comparison" className="text-sky-600 underline hover:text-sky-800">CRMの選び方</Link>でも触れていますが、移行しやすいシステムを選ぶことも重要です。</p>
      ...
      <section>
        ...
        <p>万が一の移行失敗に備えて、旧システムに戻す手順を事前に策定しておくことが重要です。<Link href="/clinic/column/clinic-line-security" className="text-sky-600 underline hover:text-sky-800">セキュリティガイド</Link>で解説しているデータ保護の観点からも、ロールバック計画は必須です。</p>
      ...
      <section>
        ...
        <p>Lオペ for CLINICでは、CSV/APIによるデータインポート機能と専門スタッフによる移行サポートを提供しています。既存の予約システム・電子カルテ・CRMからの乗り換えをご検討の場合は、まずはお気軽にご相談ください。DX導入の全体的な進め方は<Link href="/clinic/column/clinic-dx-complete-guide" className="text-sky-600 underline hover:text-sky-800">クリニックDX完全ガイド</Link>を、DX導入前後のビフォーアフターは<Link href="/clinic/column/clinic-dx-before-after" className="text-sky-600 underline hover:text-sky-800">DX導入ビフォーアフター事例</Link>もご参照ください。</p>
        ...
        <p className="text-[13px] mt-2">
          あわせて読みたい: <a href="/clinic/column/lstep-vs-clinic-tool" className="text-blue-600 underline">クリニック LINE ツール 比較 — Lス���ップ・Liny vs 専用ツールの選び方</a>
        </p>
      </section>
