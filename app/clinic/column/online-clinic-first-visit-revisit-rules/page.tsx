  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>ただし、実務上の運用は保険診療と大きく異なります。AGA治療やピル処方などの自費オンライン診療では、初診から長期処方（30〜90日分）を行うクリニックが多く存在します。<Link href="/clinic/column/self-pay-online-clinic-rules" className="text-sky-600 underline hover:text-sky-800">自費オンライン診療の制度ルールと運用ポイント</Link>で詳細を解説しています。これは指針上「必要最小限」とされる処方日数の解釈が、疾患の特性や患者の利便性を考慮した上で医師の裁量に委ねられているためです。</p>
      ...
      <section>
        ...
        <p><strong>1. 初診と再診の判定ミス。</strong>「他院での受診歴がある患者」が自院に初めて来る場合は、自院としては「初診」です。他院の紹介状がある場合でも、自院での対面診療歴がなければ初診扱いとなり、処方制限が適用されます。<Link href="/clinic/column/online-clinic-medical-record-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療のカルテ記載ガイド</Link>も踏まえ、電子カルテ上の初診/再診フラグを正しく運用し、スタッフへの周知を徹底しましょう。</p>
        ...
        <p><strong>4. 同意書の取得。</strong>オンライン診療の実施にあたっては、初診時に患者から同意書を取得する必要があります。同意事項には、オンライン診療の限界、通信トラブル時の対応、対面切り替えの可能性などを含めます。具体的な記載事項や電子同意の導入方法は<Link href="/clinic/column/online-clinic-consent-form-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療の同意書設計ガイド</Link>を参照してください。Web問診の一環として電子同意を取得するフローが効率的です。</p>
      </section>
