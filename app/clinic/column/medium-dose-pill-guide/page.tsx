  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="医薬品解説" keyPoints={keyPoints} toc={[]}>
      ...
      <section>
        ...
        <p>ざっくり言うと、<strong>「ピンポイントで短期勝負」が中用量ピル、「じっくり長期管理」が低用量ピル</strong>です。すでに低用量ピルを服用中の患者さんが月経移動を希望する場合は、実薬の延長やシート調整で対応できることも多いので、必ずしも中用量ピルに切り替える必要はありません。長期管理で副作用をさらに抑えたい場合は<Link href="/clinic/column/ultra-low-dose-pill-guide" className="text-sky-600 underline hover:text-sky-800">超低用量ピル（LEP製剤）</Link>も選択肢に入ります。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="オンライン診療との相性は抜群">
          月経移動の相談は、対面でなくても完結できるケースがほとんどです。LINEでの事前問診で血栓リスク因子を確認し、オンライン診療で処方、薬の配送——この流れを仕組み化すれば、<strong>患者さんの利便性と処方の安全性を両立</strong>できます。処方後のリマインドや副作用フォローもLINEなら手軽に行えます。婦人科のオンライン診療全般については<Link href="/clinic/column/gynecology-online-clinic-guide" className="text-sky-600 underline hover:text-sky-800">婦人科オンライン診療ガイド</Link>も参考にしてください。
        </Callout>
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="関連記事もあわせてどうぞ">
          低用量ピルの種類と選び方については<Link href="/clinic/column/pill-types-comparison" className="text-emerald-700 underline">「低用量ピルの種類と選び方ガイド」</Link>、初めてピルを飲む患者さん向けの説明資料としては<Link href="/clinic/column/low-dose-pill-beginners-guide" className="text-emerald-700 underline">「低用量ピルの飲み方完全ガイド」</Link>をご覧ください。
        </Callout>
      </section>
