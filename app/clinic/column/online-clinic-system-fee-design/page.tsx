  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="経営戦略" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>名称はクリニックによって異なり、「通信費」「オンライン診療手数料」「予約管理費」「情報通信機器使用料」など様々な呼び方があります。名称に関わらず、患者にとっては「対面では払わなかった追加費用」として認識されるため、その根拠と金額の妥当性を明確に説明できることが重要です。オンライン診療全体の<Link href="/clinic/column/online-clinic-pricing-breakdown" className="text-sky-600 underline hover:text-sky-800">料金相場と費用構造の全体像</Link>を把握した上で設計に臨みましょう。</p>
      ...
      <section>
        ...
        <p>「定額型」や「サブスク型」は、AGA治療やピル処方など<strong>月次の定期処方</strong>に適したモデルです。「月額○円ですべて込み（診察・薬・配送）」と提示できるため、患者は追加費用を心配せずに継続利用できます。<Link href="/clinic/column/self-pay-clinic-pricing-strategy" className="text-sky-600 underline hover:text-sky-800">自費クリニックの価格設定戦略</Link>も併せて参考にしてください。クリニック側にとっても月間の売上が予測しやすく、キャッシュフローが安定するメリットがあります。</p>
      </section>
