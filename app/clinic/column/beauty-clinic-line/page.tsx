  ...
  return (
    ...
    <ArticleLayout slug={self.slug} breadcrumbLabel="美容クリニック" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>美容施術において、アフターフォローは患者満足度とリピート率を左右する極めて重要な要素です。施術後の不安を解消し、適切なケアを案内することで、患者の信頼を獲得し、次回施術への意欲を高めることができます。患者の離脱を防ぐフォローアップ施策の全体像は<Link href="/clinic/column/clinic-patient-retention" className="text-sky-600 underline hover:text-sky-800">患者離脱を防ぐLINEフォローアップ</Link>もご参照ください。</p>
      ...
      <section>
        ...
        <p>美容施術の多くは、効果を維持するために定期的な再施術が必要です。施術メニューごとに推奨される再施術サイクルは異なるため、セグメント配信で適切なタイミングにリマインドを送ることがリピート率向上の鍵になります。セグメント配信の詳しい設計方法は<Link href="/clinic/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">LINEセグメント配信でリピート率を向上</Link>で解説しています。</p>
      ...
      <section>
        ...
        <p>美容クリニック選びにおいて、口コミは患者の意思決定に最も大きな影響を与える要因の一つです。なお、配信のしすぎでブロックされてしまっては元も子もありません。<Link href="/clinic/column/line-block-rate-reduction" className="text-sky-600 underline hover:text-sky-800">ブロック率を下げる5つの鉄則</Link>を押さえた上で口コミ促進を進めましょう。</p>
