    ...
    <ArticleLayout slug={self.slug} breadcrumbLabel="オンライン問診" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <Callout type="success" title="LINEで問診を送る利点">
          <p>メールの開封率20〜30%に対し、LINEの開封率は80%超。問診の回答率も必然的に高くなります。また、患者は新たなアプリをインストールする必要がなく、普段使い慣れたLINEの画面から問診フォームにアクセスできます。問診後の予約管理と組み合わせるなら<Link href="/clinic/column/reservation-system-comparison" className="text-blue-600 underline">予約システム比較10選</Link>も参考にしてください。</p>
        </Callout>
      </section>
      ...
      <section>
        ...
        <Callout type="success" title="データ活用の事例">
          <p>ある内科クリニックでは、オンライン問診データの分析から「花粉症の問い合わせが2月上旬から急増する」という傾向を発見。1月下旬にLINE配信を行ったところ、花粉症シーズンの来院数が<strong>前年比130%</strong>に増加しました。このようなデータ活用をはじめとするクリニックのDX推進については<Link href="/clinic/column/clinic-dx-guide" className="text-blue-600 underline">クリニックDX完全ガイド</Link>で体系的に解説しています。</p>
        </Callout>
      </section>
      ...
      <section>
        ...
        <p className="mt-4">オンライン問診で得たデータを電子カルテと連携させることで、さらなる業務効率化が実現します。連携のポイントは<Link href="/clinic/column/electronic-medical-record-guide" className="text-blue-600 underline">電子カルテ選び方ガイド</Link>をご覧ください。Lオペ for CLINICは、LINE友だち追加から問診送付、<Link href="/clinic/features#予約・診察" className="text-sky-600 underline hover:text-sky-800">予約連携</Link>までをワンストップで実現するオンライン問診機能を提供しています。紙の問診票からの移行を検討されている方は、ぜひお気軽にご相談ください。</p>
      </section>
