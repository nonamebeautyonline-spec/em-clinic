...
const faqItems = [
  { q: "リピート処方のストック収益モデルとは何ですか？", a: "毎月継続的に処方が発生する薬剤（AGA治療薬・低用量ピル・GLP-1ダイエット薬など）を定期配送の仕組みに組み込み、月額課金型の安定収益を構築するモデルです。SaaSの月額課金と同じ考え方で、新規獲得患者が毎月積み上がり、解約率を低く保つことで収益が成長し続けます。" },
  { q: "リピート処方の解約率はどのくらいを目標にすべきですか？", a: "月間解約率3%以下が目標です。5%と3%では12か月後の継続患者数に約1.3倍の差が生まれます。解約率を下げるには、完全解約ではなく一時停止の選択肢を用意すること、長期継続インセンティブ（6か月で10%OFF等）を設定すること、LINEの自動フォローで不安を早期解消することが効果的です。" },
  { q: "定期配送の仕組みをクリニックで導入するにはどうすればよいですか？", a: "初回診察で処方内容を決定し、決済情報の登録と定期配送の同意を取得します。月間100件以下であればクリニック内で梱包・発送が可能で、それ以上は配送代行サービス（1件300〜500円程度）の活用が効率的です。3〜6か月ごとにオンライン診察で経過確認を行い、安全管理と継続率の両立を図ります。" },
];
  ...
  return (
    <ArticleLayout slug={slug} breadcrumbLabel="経営戦略" keyPoints={keyPoints} toc={toc}>
      ...
      <p className="text-[15px] leading-relaxed text-gray-700 font-medium bg-blue-50 rounded-xl p-5 border border-blue-100">
        自費クリニックの収益は新患の来院に依存しがちですが、<strong>リピート処方による「ストック型収益」</strong>を構築すれば、毎月の売上基盤が安定します。AGA治療薬・ピル・ダイエット薬・スキンケア処方など、継続服用が前提の自費処方は<strong>定期配送＋自動フォローの仕組み化</strong>で収益を積み上げる最適な商材です。本記事では、リピート処方のストック収益モデルの設計から、解約率を最小化する運用ノウハウまでを解説します。サブスクリプションモデル全般については<Link href="/clinic/column/self-pay-clinic-subscription-model" className="text-sky-600 underline hover:text-sky-800">サブスクモデル設計ガイド</Link>もあわせてご覧ください。
      </p>
      ...
      <section>
        ...
        <p>
          <strong>AGA治療と低用量ピル</strong>はリピート処方の「王道」です。いずれも服用を中止すると効果が失われるため、患者自身が継続を強く望みます。また、経過が安定していればオンライン診療で処方更新が可能なため、来院不要の定期配送モデルと相性が抜群です。美容内服の定期配送で収益を伸ばす具体例は<Link href="/clinic/column/beauty-oral-subscription-revenue" className="text-sky-600 underline hover:text-sky-800">美容内服サブスクの収益モデル</Link>で紹介しています。クリニック側にとっても、診療1回あたりの所要時間が短い（5〜10分）ため、効率的に多くの患者を対応できます。
        </p>
      ...
      <section>
        ...
        <p>
          自費クリニックの経営を安定させるカギは、<strong>新患獲得への依存を減らし、リピート処方のストック収益を育てる</strong>ことです。月30人の新規獲得を12か月継続すれば、月250万円の安定収益基盤が構築できます。このモデルの構築に必要なのは、オンライン診療・LINE連携・定期配送の3つの仕組みです。<Link href="/clinic/column/self-pay-clinic-subscription-model" className="text-sky-600 underline hover:text-sky-800">サブスクモデルの全体設計</Link>と組み合わせることで、リピート処方以外のストック収益源も同時に構築できます。いずれもLオペ for CLINICでカバー可能です。
        </p>

        <h3 className="text-lg font-semibold text-gray-700 mt-6">関連記事</h3>
        <ul className="space-y-1 text-gray-700">
          <li>
            <Link href="/clinic/column/self-pay-clinic-subscription-model" className="text-sky-600 underline hover:text-sky-800">サブスクモデル設計ガイド</Link> — 月額課金モデルの全体設計
          </li>
          <li>
            <Link href="/clinic/column/self-pay-clinic-break-even" className="text-sky-600 underline hover:text-sky-800">損益分岐点と収支管理</Link> — ストック収益が経営指標に与える影響
          </li>
          <li>
            <Link href="/clinic/column/clinic-repeat-rate-improvement" className="text-sky-600 underline hover:text-sky-800">リピート率改善ガイド</Link> — 継続率を高める施策の全体像
          </li>
          <li>
            <Link href="/clinic/column/self-pay-patient-ltv-maximize" className="text-sky-600 underline hover:text-sky-800">患者LTVの最大化</Link> — 1人あたりの生涯売上を高める方法
          </li>
          <li>
            <Link href="/clinic/contact" className="text-sky-600 underline hover:text-sky-800">無料相談・お問い合わせ</Link> — リピート処方モデルのご相談はこちら
          </li>
        </ul>
      </section>
