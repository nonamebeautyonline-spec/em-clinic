...
const faqItems = [
  { q: "クリニックのGoogle口コミ対策の効果はどのくらいで実感できますか？", a: "施策にもよりますが、LINE配信やSEO対策は1〜3ヶ月で効果が出始めるケースが多いです。特にセグメント配信は導入直後から開封率・クリック率の改善が見られます。継続的な改善サイクルを回すことで、半年後には大きな成果に繋がります。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>特に新規開業や競合が多いエリアでは、口コミの数と質が集患力に直結します。しかし、多くのクリニックでは口コミ対策を「待つだけ」にしており、能動的な施策を行っていません。<Link href="/clinic/column/clinic-line-friends-growth" className="text-sky-600 underline hover:text-sky-800">LINE友だち集めの施策</Link>と口コミ対策を組み合わせることで、相乗効果が得られます。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="info" title="口コミ依頼のベストタイミング">
          診察後24〜48時間が口コミ依頼のゴールデンタイムです。患者の診察体験が新鮮なうちに依頼することで、回答率が2倍以上向上します。<Link href="/clinic/column/clinic-nps-survey" className="text-sky-600 underline hover:text-sky-800">NPS調査の導入方法</Link>と組み合わせると、さらに効果的です。
        </Callout>
      ...
      <section>
        ...
        <ul className="space-y-2 text-gray-700">
          ...
          <li><strong>LINE上でのフォロー</strong>：口コミを書いた患者がLINE友だちの場合、個別メッセージで丁寧にフォローアップ。<Link href="/clinic/column/clinic-reputation-crisis-management" className="text-sky-600 underline hover:text-sky-800">口コミ炎上対策</Link>の記事も参考にしてください</li>
        </ul>
      ...
      <section>
        ...
        <p><Link href="/clinic/column/clinic-patient-communication" className="text-sky-600 underline hover:text-sky-800">患者コミュニケーションの改善</Link>は、口コミ評価の向上にも直結します。日頃の患者対応の質を高めることが、最も効果的なMEO対策です。</p>
      </section>
