...
const faqItems = [
  { q: "クリニックDXで1日の業務がここまで変わるの導入にどのくらいの期間がかかりますか？", a: "基本的な設定は1〜2週間で完了します。LINE公式アカウントの開設からリッチメニュー設計・自動メッセージ設定まで、Lオペ for CLINICなら初期設定サポート付きで最短2週間で運用開始できます。" },
  { q: "クリニックDXで1日の業務がここまで変わるでスタッフの負荷は増えませんか？", a: "むしろ減ります。電話対応・手動での予約管理・問診確認などの定型業務を自動化することで、スタッフの作業時間を月40時間以上削減できた事例もあります。導入初月はサポートを受けながら進めれば、2ヶ月目以降はスムーズに運用できます。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="業務改善" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>DXの本質は「ITツールを入れること」ではなく、<strong>スタッフと院長が本来の仕事に集中できる環境を作ること</strong>です。Lオペは問診・予約・会計・フォローアップの全工程をLINE上で一元管理することで、この理想を実現します。詳しい機能については<Link href="/clinic/column/clinic-dx-complete-guide" className="text-sky-600 underline hover:text-sky-800">クリニックDX完全ガイド</Link>でも解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>従来の手書き問診票は、患者の記入に10〜15分、スタッフの転記に5分、合計で1人あたり最大20分かかっていました。LINE事前問診では、患者が来院前日にスマートフォンで回答を完了するため、来院時の問診工程がほぼゼロになります。<Link href="/clinic/column/online-questionnaire-guide" className="text-sky-600 underline hover:text-sky-800">オンライン問診導入ガイド</Link>でさらに詳しく解説しています。</p>
        ...
        <p>売上への具体的なインパクトについては<Link href="/clinic/column/clinic-line-revenue-growth" className="text-sky-600 underline hover:text-sky-800">LINE活用でクリニックの売上を伸ばす方法</Link>で詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <p>この一連のフローがすべてLINE上で完結するため、患者にとっては「LINEを開くだけ」でクリニックとの全てのやり取りが完了します。クリニック側も一つのダッシュボードで全業務を管理できるため、ツールの乱立による非効率が生まれません。待ち時間の削減効果については<Link href="/clinic/column/clinic-waiting-time" className="text-sky-600 underline hover:text-sky-800">クリニックの待ち時間対策</Link>も併せてご覧ください。</p>
