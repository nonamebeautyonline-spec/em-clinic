...
const faqItems = [
  { q: "自費診療の患者LTV最大化ガイドで売上を伸ばす最も効果的な方法は？", a: "既存患者へのセグメント配信が最も即効性があります。来院履歴・診療内容に基づいて、関連する自費メニューをLINEで個別提案することで、押し売り感なく自費転換率を高められます。導入クリニックでは自費率が15%→35%に向上した事例もあります。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>上記は自費診療の主要カテゴリにおける平均的なLTVの目安です。AGA治療やダイエット外来は継続期間が長いためLTVが高くなりやすく、花粉症治療は季節性のため相対的に低くなる傾向があります。ただし、花粉症から<strong>舌下免疫療法へのアップセル</strong>が成功すれば、LTVは大幅に向上します。LTVの基本的な考え方は<Link href="/clinic/column/clinic-patient-ltv" className="text-sky-600 underline hover:text-sky-800">クリニックの患者LTV向上戦略</Link>でも詳しく解説しています。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="LTV15万円と75万円では年間売上に5倍の差">
          同じ新患数でも、LTVを5倍に引き上げれば売上も5倍になります。新患獲得だけに注力するのではなく、<strong>獲得した患者の価値を最大化する仕組み</strong>を構築することが、自費診療クリニック経営の最重要テーマです。自費診療の収益構造については<Link href="/clinic/column/clinic-self-pay-revenue" className="text-sky-600 underline hover:text-sky-800">自費診療の収益最大化ガイド</Link>もご覧ください。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>LTV向上の第一歩は、<strong>初回来院の体験設計</strong>です。初診の満足度が高ければ2回目の来院率は大幅に上がり、逆に不満があれば二度と来院しません。自費診療では特に「高い費用を払ったのに期待した体験が得られなかった」という不満が離脱の最大要因です。カウンセリング体験の質を高めて初回成約率を上げる方法は<Link href="/clinic/column/self-pay-clinic-counseling-conversion" className="text-sky-600 underline hover:text-sky-800">カウンセリング成約率アップガイド</Link>で、価格設定の最適化による単価向上は<Link href="/clinic/column/self-pay-clinic-pricing-strategy" className="text-sky-600 underline hover:text-sky-800">価格設定戦略ガイド</Link>で解説しています。</p>
      ...
      <section>
        ...
        <p>リピート率35%→75%の改善は、LTVに換算すると<strong>約2.1倍の向上</strong>を意味します。リピート率改善の詳しいノウハウは<Link href="/clinic/column/clinic-repeat-rate-improvement" className="text-sky-600 underline hover:text-sky-800">クリニックのリピート率改善ガイド</Link>も参照してください。</p>
      </section>
      ...
      <section>
        ...
        <p>自費診療の収益を最大化する方法は<Link href="/clinic/column/self-pay-clinic-revenue-triple" className="text-sky-600 underline hover:text-sky-800">自費診療クリニックの売上を3倍にする方法</Link>でも詳しく解説しています。クロスセル・アップセルは、患者の悩みを包括的に解決するという点で、<strong>患者体験の向上と収益向上を両立</strong>できる数少ない施策です。</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-1 text-gray-700">
          <li>・<Link href="/clinic/column/clinic-patient-ltv" className="text-sky-600 underline hover:text-sky-800">クリニックの患者LTV向上戦略</Link></li>
          <li>・<Link href="/clinic/column/clinic-self-pay-revenue" className="text-sky-600 underline hover:text-sky-800">自費診療の収益最大化ガイド</Link></li>
          <li>・<Link href="/clinic/column/self-pay-clinic-revenue-triple" className="text-sky-600 underline hover:text-sky-800">自費診療クリニックの売上を3倍にする方法</Link></li>
          <li>・<Link href="/clinic/column/clinic-repeat-rate-improvement" className="text-sky-600 underline hover:text-sky-800">クリニックのリピート率改善ガイド</Link></li>
        </ul>

        <p className="mt-4">LTV最大化の仕組みづくりについて、詳しくは<Link href="/clinic/contact" className="text-sky-600 underline hover:text-sky-800">お問い合わせページ</Link>からお気軽にご相談ください。</p>
      </section>
