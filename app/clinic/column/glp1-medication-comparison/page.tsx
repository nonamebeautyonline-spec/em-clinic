...
const faqItems = [
  { q: "GLP-1受容体作動薬の種類比較はオンライン診療で処方できますか？", a: "多くの場合、オンライン診療での処方が可能です。ただし初診では処方日数に制限がある場合があります。再診であれば対面診療と同等の処方が可能です。詳しくは各薬剤の処方ルールをご確認ください。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="医薬品解説" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>患者フォローでは、増量前に必ず副作用の有無を確認し、消化器症状が持続している場合は増量を見送る判断が必要です。LINEでの定期フォローにより、「悪心がひどいので自己判断で中止した」というケースを防ぎ、適切な対処（増量見送り、制吐剤の処方等）を提案できます。リベルサスの副作用と対処法の詳細は<Link href="/clinic/column/rybelsus-side-effects-guide" className="text-sky-600 underline hover:text-sky-800">リベルサスの副作用ガイド</Link>を参照してください。</p>
      </section>
      ...
      <section>
        ...
        <p><Link href="/clinic/column/diet-glp1-online-clinic-lope" className="text-emerald-700 underline">メディカルダイエット（GLP-1）のオンライン診療ガイド</Link>や<Link href="/clinic/column/diet-online-clinic-winning-strategy" className="text-emerald-700 underline">メディカルダイエットクリニックの運営戦略</Link>も参考に、安全性と効率性を両立するオンライン処方体制を構築してください。</p>
      </section>
