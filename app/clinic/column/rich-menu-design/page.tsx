...
const self = articles.find((a) => a.slug === "rich-menu-design")!;
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="リッチメニュー" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>LINE公式アカウントの標準機能では1パターンのリッチメニューしか設定できませんが、Lオペのようなツールを使えば、患者の状態に応じて<strong>複数のリッチメニューを動的に切り替える</strong>ことが可能になります。なお、友だち数を効率よく増やす施策については<Link href="/clinic/column/clinic-line-friends-growth" className="text-sky-600 underline hover:text-sky-800">LINE友だち集め月100人増やす7つの施策</Link>をご覧ください。</p>
      </section>
      ...
      <section>
        ...
        <p>リッチメニューは作って終わりではなく、<strong>継続的にテスト・改善するもの</strong>です。配信施策全体のブロック率を抑える工夫については<Link href="/clinic/column/line-block-rate-reduction" className="text-sky-600 underline hover:text-sky-800">ブロック率を下げる5つの鉄則</Link>も併せて確認しておくと効果的です。以下の要素をA/Bテストし、クリック率やコンバージョン率を改善していきましょう。</p>
