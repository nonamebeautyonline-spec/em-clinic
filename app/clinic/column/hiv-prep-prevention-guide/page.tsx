...
const faqItems = [
  { q: "HIV PrEP完全ガイドはオンライン診療で処方できますか？", a: "多くの場合、オンライン診療での処方が可能です。ただし初診では処方日数に制限がある場合があります。再診であれば対面診療と同等の処方が可能です。詳しくは各薬剤の処方ルールをご確認ください。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="医薬品解説" keyPoints={keyPoints} toc={[]}>
      ...
      <section>
        ...
        <p>大切なのは、<strong>定期的な検査とフォローアップを欠かさないこと</strong>。PrEPはHIV以外のSTIは防げませんし、腎機能のモニタリングも必須です。細菌性STIの事後予防としては<Link href="/clinic/column/doxy-pep-std-prevention" className="text-sky-600 underline hover:text-sky-800">ドキシペップ（Doxy-PEP）</Link>という選択肢も登場しており、PrEPとの併用が注目されています。また、急増する梅毒の検査・治療については<Link href="/clinic/column/syphilis-diagnosis-treatment-guide" className="text-sky-600 underline hover:text-sky-800">梅毒の診断・治療ガイド</Link>をご覧ください。オンライン診療を活用すれば、これらのフォローアップを無理なく継続できます。STD領域でのオンライン診療の始め方は<Link href="/clinic/column/std-online-clinic-lope" className="text-sky-600 underline hover:text-sky-800">STDオンライン診療の始め方</Link>で解説しています。HIV予防の選択肢を広げることは、患者さんの人生を守ることにつながります。</p>
      </section>
