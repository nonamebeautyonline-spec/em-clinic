...
async function getScenarioDetailStats(scenarioId: number, tenantId: string | null) {
  ...
  const activeCount = allEnrollments.filter((e: EnrollmentRow) => e.status === "active").length;
  const completedCount = allEnrollments.filter((e: EnrollmentRow) => e.status === "completed").length;
  const exitedCount = allEnrollments.filter((e: EnrollmentRow) => e.status === "exited").length;
  const pausedCount = allEnrollments.filter((e: EnrollmentRow) => e.status === "paused").length;
  ...
  const funnel = allSteps.map((step: StepRow) => {
    // そのステップに到達した人 = current_step_order >= sort_order、または完了者
    const reached = allEnrollments.filter((e: EnrollmentRow) => {
      // 完了者は全ステップを通過
...
async function getAllScenariosStats(tenantId: string | null) {
  ...
  // シナリオ別の集計
  const scenarioStats = allScenarios.map((s: ScenarioRow) => {
    const sEnrollments = allEnrollments.filter((e) => e.scenario_id === s.id);
    const total = sEnrollments.length;
    const completed = sEnrollments.filter((e) => e.status === "completed").length;
    const exited = sEnrollments.filter((e) => e.status === "exited").length;
    const active = sEnrollments.filter((e) => e.status === "active").length;
 ...
 * ステップのラベルを生成
 */
function getStepLabel(step: StepRow): string {
 ...
 */
function buildMonthlyTrend(enrollments: EnrollmentRow[]): { month: string; enrolled: number; completed: number; exited: number }[] {
  const now = new Date();
