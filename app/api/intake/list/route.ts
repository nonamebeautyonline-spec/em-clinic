...
// ★ intake_completion モード: 問診完了ベースで直接取得（予約不要）
async function handleIntakeCompletionMode(tenantId: string, fromDate: string | null, toDate: string | null) {
  try {
    // intakeテーブルから問診完了済みレコードを直接取得（answersが存在するもの）
    let intakeData: IntakeRow[];

    if (fromDate && toDate) {
      // created_at の日付範囲でフィルタ（JSTベース）
      const { data, error } = await fetchAll(() =>
        strictWithTenant(
          supabaseAdmin
            .from("intake")
      ...
      if (error) {
        console.error("[intake/list:intake_completion] query error:", error);
        return NextResponse.json({ ok: false, error: "DB_ERROR", detail: error.message }, { status: 500 });
      }
      // answers が空オブジェクト {} のレコードを除外
    ...
    // 結合してレスポンス構築
    // ★ reserved_date / reserved_time に created_at の日付・時刻を入れる
    //   → フロントの日付フィルタ・ソートがそのまま機能する
    const rows = intakeData.map((row: IntakeRow) => {
      const pat = row.patient_id ? patientsMap.get(row.patient_id) : null;
      const patientName = pat?.name || "";

      // created_at を JST の日付・時刻に分解
    ...
    console.log(`[intake/list:intake_completion] Retrieved ${rows.length} rows`);
    return NextResponse.json({ ok: true, rows, karteMode: "intake_completion" });
  } catch (err) {
    console.error("intake list (intake_completion) API error:", err);
    return serverError(String(err));
  }
}
