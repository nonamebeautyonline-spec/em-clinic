  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="活用事例" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>ポイントは、対面施術の補完として処方する薬剤をメニュー化しておくことです。たとえばレーザー治療後の色素沈着予防としてトラネキサム酸＋ハイドロキノンを処方し、施術後1〜3ヶ月はオンラインで処方を継続するフローを標準化すれば、患者にとっては「施術効果の維持」、クリニックにとっては「継続課金」の双方にメリットがあります。</p>

        <p>外用薬の処方にあたっては、使用方法の動画をあらかじめLINEで配信しておくと、オンライン診療時の説明時間を短縮でき、患者の使用方法の誤りも防止できます。<Link href="/lp/column/online-clinic-complete-guide" className="text-emerald-700 underline">オンライン診療の全体像</Link>と合わせて運用設計を行いましょう。</p>
      </section>
      ...
      <section>
        ...
        <Callout type="point" title="セットメニューの効果">
          「レーザー施術＋美白内服3ヶ月セット」のように対面施術とオンライン処方をパッケージ化して提示すると、患者は「3ヶ月間ケアしてもらえる」という安心感を得られます。単体メニューよりもセットの方が処方継続率が高くなる傾向があり、客単価の向上にも寄与します。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p><Link href="/lp/column/clinic-line-automation" className="text-emerald-700 underline">LINE自動化の詳細</Link>で解説している通り、Lオペのセグメント配信機能を使えば、施術内容や処方薬に応じて配信内容を出し分けることも可能です。レーザー施術を受けた患者にはダウンタイムの注意点を、ピーリングを受けた患者には保湿ケアの重要性を、それぞれ適切なタイミングで届けられます。</p>
      ...
      <section>
        ...
        <p>美容皮膚科のハイブリッド運用においては、オンライン診療のガイドラインと医療広告ガイドラインの双方を遵守する必要があります。オンライン処方については、<Link href="/lp/column/online-clinic-prescription-rules" className="text-emerald-700 underline">オンライン診療の処方ルール</Link>で解説した通り、初診からオンラインで処方できる薬剤と、対面診察を経てからオンラインに移行すべき薬剤があります。</p>
        ...
        <p>オンラインでの処方薬の広告についても同様に、医薬品医療機器等法の規定に従い、未承認薬を扱う場合はその旨を明記し、リスクと副作用情報を適切に提供する必要があります。<Link href="/lp/column/medical-ad-guideline-compliance" className="text-emerald-700 underline">医療広告ガイドラインの詳細</Link>も確認しておきましょう。</p>
      </section>
      ...
      <section>
        ...
        <p>導入のポイントは、①オンライン処方に適した薬剤メニューの整備、②施術後フォローのフロー標準化、③LINEを活用した自動化の3点に集約されます。これらを段階的に構築していくことで、既存の対面施術の売上を維持しながら、処方のストック収益を上乗せできる経営構造が完成します。</p>
