  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ツール比較" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p><strong>3. LINE/SNS連携型。</strong>患者が普段使っているLINEをインターフェースとして活用し、予約・問診・ビデオ通話・決済・配送管理までをLINE上で完結させるモデルです。患者にアプリの追加インストールを求めないため、導入のハードルが最も低くなります。LINE連携型と既存ツールの違いについては<Link href="/lp/column/lstep-vs-clinic-tool" className="text-sky-600 underline hover:text-sky-800">Lステップとクリニック専用ツールの比較</Link>も参考になります。</p>
      ...
      <section>
        ...
        <Callout type="info" title="隠れコストに注意">
          表示されている月額料金だけでなく、<strong>決済手数料（3.0〜3.6%）、SMS送信料（1通3〜10円）、ストレージ追加料金</strong>などの従量課金も年間コストに含めて比較しましょう。<Link href="/lp/column/online-clinic-pricing-breakdown" className="text-sky-600 underline hover:text-sky-800">オンライン診療の料金相場と隠れコスト</Link>の記事で費用構造の全体像を把握できます。月間100件・平均単価1万円の場合、決済手数料だけで年間36〜43万円のコストになります。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p><strong>自費診療特化・オンライン専門のクリニック</strong>は、予約・問診・決済・配送を一気通貫で管理できるシステムが最適です。<Link href="/lp/column/online-clinic-platform-comparison" className="text-sky-600 underline hover:text-sky-800">オンライン診療プラットフォームの詳細比較</Link>も選定の参考にしてください。特にLINE連携型は、患者のアプリDLが不要で予約導線がシンプルなため、新患獲得と継続率の両面で優位性があります。</p>
