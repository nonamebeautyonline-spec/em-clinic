  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="活用事例" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>この「巨大な潜在市場」と「オンライン診療との高い親和性」が、AGA治療をオンライン診療の最有望領域に押し上げています。<Link href="/lp/column/online-clinic-complete-guide" className="text-emerald-700 underline">オンライン診療の全体ガイド</Link>でも解説している通り、規制緩和の追い風も重なり、AGA特化型のオンラインクリニックは今後さらに増加する見通しです。</p>
      </section>
      ...
      <section>
        ...
        <p>実際の処方では、フィナステリド（またはデュタステリド）とミノキシジルを<strong>併用するコンビネーション療法</strong>が標準的です。「守り（抜け毛抑制）」と「攻め（発毛促進）」の両面からアプローチすることで、単剤よりも高い効果が得られます。この併用パターンごとに月額の処方プランを用意しておくことが、<Link href="/lp/column/clinic-self-pay-revenue" className="text-emerald-700 underline">自費診療の売上最大化</Link>につながります。</p>
      </section>
      ...
      <section>
        ...
        <p>注目すべきは<strong>患者LTV（生涯価値）が12〜24万円</strong>に達する点です。月額1万円の処方を18ヶ月継続すれば18万円。利益率70%で計算すると、患者1人あたりの粗利は約12.6万円です。この高いLTVがAGA治療の経営的な魅力の根幹にあります。<Link href="/lp/column/clinic-line-revenue-growth" className="text-emerald-700 underline">クリニックのLINE活用売上改善</Link>の観点からも、AGA治療は最も収益性の高いオンライン診療領域の一つです。</p>
      ...
      <section>
        ...
        <p>治療開始1ヶ月目、3ヶ月目、6ヶ月目など、経過に応じた情報をLINEで自動配信します。「治療開始3ヶ月目は初期脱毛が落ち着く時期です」「6ヶ月で多くの方が効果を実感しています」といった<strong>治療経過に寄り添うメッセージ</strong>で、継続モチベーションを維持します。<Link href="/lp/column/segment-delivery-repeat" className="text-emerald-700 underline">セグメント配信の詳細</Link>もあわせてご参照ください。</p>
