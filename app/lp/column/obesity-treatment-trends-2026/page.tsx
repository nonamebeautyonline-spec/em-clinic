...
const faqItems = [
  { q: "肥満症治療の最新動向はオンライン診療で処方できますか？", a: "多くの場合、オンライン診療での処方が可能です。ただし初診では処方日数に制限がある場合があります。再診であれば対面診療と同等の処方が可能です。詳しくは各薬剤の処方ルールをご確認ください。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>上記の通り、同じGLP-1受容体作動薬でも<strong>承認適応は薬剤ごとに異なります</strong>。日本で肥満症治療薬として正式に承認されているのはウゴービのみであり、オゼンピック・ビクトーザは2型糖尿病治療薬、サクセンダ・ゼップバウンドは日本では未承認です。各GLP-1製剤の効果・費用・副作用の違いについては<Link href="/lp/column/glp1-medication-comparison" className="text-sky-600 underline hover:text-sky-800">GLP-1薬剤比較ガイド</Link>で詳しく整理しています。</p>
      </section>
      ...
      <section>
        ...
        <p>近年、SNSや美容系メディアでメディカルダイエット（GLP-1）が注目を集め、自費クリニックでの処方が急増しました。<Link href="/lp/column/diet-online-clinic-winning-strategy" className="text-sky-600 underline hover:text-sky-800">メディカルダイエット戦略</Link>として参入するクリニックも増えていますが、日本糖尿病学会は2023年に<strong>「GLP-1受容体作動薬の適応外使用に関する見解」</strong>を公表し、糖尿病治療薬としての安定供給への影響や、医学的管理が不十分な処方に対して懸念を表明しています。</p>
      ...
      <section>
        ...
        <p>第三に、<strong>オンライン診療との組み合わせ</strong>の拡大です。肥満症の継続治療は長期にわたるため、通院負担の軽減は治療継続率に直結します。ウゴービの保険処方には施設基準が求められますが、安定期のフォローアップにオンライン診療を活用する運用は今後増加していくと考えられます。肥満症に限らず、<Link href="/lp/column/lifestyle-disease-online-management" className="text-sky-600 underline hover:text-sky-800">生活習慣病全般のオンライン管理</Link>は慢性疾患管理の重要な柱です。</p>
      </section>
