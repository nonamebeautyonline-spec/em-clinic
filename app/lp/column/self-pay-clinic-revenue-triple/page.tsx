  ...
  return (
    <ArticleLayout slug={SLUG} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>では、各変数を具体的にどう改善するのか。ここからは3つの戦略軸それぞれについて、実践的な施策を解説していきます。<Link href="/lp/column/clinic-line-revenue-growth" className="text-emerald-700 underline">クリニックの売上を上げるLINE活用術</Link>もあわせてご覧ください。</p>
      </section>
      ...
      <section>
        ...
        <p>既存患者からの紹介は、最も獲得単価が低く、かつ成約率が高い集患方法です。紹介された患者は最初から信頼度が高く、リピート率も高い傾向にあります。紹介者と被紹介者の双方にメリットのある特典設計が成功の鍵です。<Link href="/lp/column/clinic-referral-program" className="text-emerald-700 underline">LINE紹介プログラム</Link>を活用すれば、紹介の発生から特典付与まで自動化できます。</p>
        ...
        <p>特に重要なのは、ファネルの中間地点（MOFU）で<strong>LINE友だち追加を獲得する</strong>ことです。友だち追加さえしてもらえれば、その後のフォローアップはセグメント配信で自動化できます。<Link href="/lp/column/clinic-line-friends-growth" className="text-emerald-700 underline">LINE友だちを効率的に増やす方法</Link>も参考にしてください。</p>
      ...
      <section>
        ...
        <p>単価アップで最も注意すべきは、「患者に提供する価値も同時に上がっているか」です。セットメニューは医学的な根拠に基づいた組み合わせであるべきですし、コースは継続することで明確な効果が期待できるものでなければなりません。<strong>価値の再設計なくして値上げは成立しない</strong>——この原則を忘れないでください。<Link href="/lp/column/clinic-self-pay-revenue" className="text-emerald-700 underline">自費診療の売上を伸ばすLINE配信戦略</Link>も合わせてご参照ください。</p>
      </section>
      ...
      <section>
        ...
        <p>しかし、自費診療の患者は保険診療と異なり「定期的に通う動機」が弱いことが多く、1回施術を受けて満足すると離脱してしまうケースが少なくありません。ここでLINEフォローアップの出番です。</p>
        ...
        <p>AGA治療薬、ダイエット薬、スキンケア製品など、定期的な購入・処方が必要な治療では、「薬がなくなるタイミング」での自動リマインドが極めて効果的です。処方日数から逆算して配信タイミングを自動設定するため、スタッフの手間はゼロ。にもかかわらず、<strong>定期処方の継続率は平均30%向上</strong>します。</p>
        ...
        <p>リピート率向上は、売上の「掛け算」で効いてくるため、3倍成長において最もレバレッジが大きい変数です。特に自費クリニックでは、1人の患者のLTVが高いため、リピート率を10%改善するだけで年間数百万円のインパクトがあります。<Link href="/lp/column/clinic-patient-ltv" className="text-emerald-700 underline">患者LTVを最大化する方法</Link>の記事でも、具体的な計算方法と改善施策を解説しています。</p>
