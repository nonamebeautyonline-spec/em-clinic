  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="活用事例" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>美容クリニックチェーンの調査では、再診率が10ポイント向上すると年間売上が20〜30%増加するというデータがあります。特にLTV（顧客生涯価値）が高い施術カテゴリ（AGA、美容内服、ダイエット処方など）では、再診率の影響がさらに大きくなります。<Link href="/lp/column/clinic-line-case-studies" className="text-sky-600 underline hover:text-sky-800">LINE活用の成功事例</Link>でも、再診率改善が経営に与えるインパクトが具体的に示されています。</p>
      </section>
      ...
      <section>
        ...
        <p>セグメント配信とは、患者を属性や行動に基づいてグループ分けし、各グループに最適なメッセージを届ける手法です。全員に同じメッセージを一斉配信する方法と比べ、セグメント配信はクリック率が2〜3倍、再診率への転換率が1.5〜2倍になるというデータがあります。<Link href="/lp/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">セグメント配信の基本</Link>も合わせて参照してください。</p>
