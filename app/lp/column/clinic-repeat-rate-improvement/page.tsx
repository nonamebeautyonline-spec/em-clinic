  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>注目すべきは、<strong>最大の理由が「忘れていた」（40%）</strong>であること。つまり、不満があったわけでもなく、競合に奪われたわけでもなく、単に「思い出すきっかけがなかった」だけで再来院に至っていないのです。患者LTVの観点からこの離脱コストを試算する方法は<Link href="/lp/column/clinic-patient-ltv" className="text-sky-600 underline hover:text-sky-800">患者LTV向上戦略</Link>で詳しく解説しています。</p>
      ...
      <section>
        ...
        <p>季節配信の詳細なノウハウは<Link href="/lp/column/segment-delivery-repeat" className="text-emerald-700 underline">LINEセグメント配信でリピート率を向上させる方法</Link>で解説しています。</p>
      ...
      <section>
        ...
        <p>リピーターは新患と比べて単価が高い傾向にあり、<Link href="/lp/column/clinic-patient-retention" className="text-emerald-700 underline">患者離脱を防ぐLINEフォローアップ</Link>と組み合わせることで、LTV（顧客生涯価値）を大幅に向上させることができます。</p>
      ...
      <section>
        ...
        <p>結果、再処方率が<strong>52%から74%</strong>に向上。患者からは「LINEで気軽に相談できるので安心して通える」という声が多く寄せられ、<Link href="/lp/column/clinic-nps-survey" className="text-emerald-700 underline">NPS（患者満足度スコア）</Link>も15ポイント向上しました。</p>
        ...
        <p>3つの事例に共通するのは、<strong>リマインドメッセージ経由の再来院率が72%</strong>に達しているという点です。つまり、自動フォローを受けた患者の約7割が実際に再来院しています。フォローメッセージは「営業」ではなく「気遣い」として受け取られるため、<Link href="/lp/column/clinic-line-revenue-growth" className="text-emerald-700 underline">売上向上</Link>と患者満足度の両立が可能です。</p>
      </section>
      ...
      <section>
        ...
        <p>手動でのフォローアップ（電話・ハガキ・メール作成）に月間約30時間かかっていた作業が、Lオペの自動化により<strong>月2時間</strong>にまで削減されます。残りの28時間を診療や他の業務改善に充てることで、クリニック全体の生産性が向上します。LINE自動化の設定方法の詳細は<Link href="/lp/column/clinic-line-automation" className="text-sky-600 underline hover:text-sky-800">LINE自動化完全ガイド</Link>もご参照ください。</p>
      ...
      <section>
        ...
        <ul className="mt-3 space-y-2 text-[15px]">
          <li>
            <Link href="/lp/column/segment-delivery-repeat" className="text-emerald-700 underline">LINEセグメント配信でリピート率を向上させる方法</Link> — セグメント設計の詳細ノウハウ
          </li>
          <li>
            <Link href="/lp/column/clinic-patient-retention" className="text-emerald-700 underline">クリニックの患者離脱を防ぐLINEフォローアップ</Link> — 離脱防止の具体策
          </li>
          <li>
            <Link href="/lp/column/clinic-line-revenue-growth" className="text-emerald-700 underline">クリニックの売上を上げるLINE活用術</Link> — 再診率・自費率・新患獲得の3軸で収益改善
          </li>
          <li>
            <Link href="/lp/column/clinic-nps-survey" className="text-emerald-700 underline">クリニックのNPS調査導入ガイド</Link> — 患者満足度の可視化方法
          </li>
        </ul>

        <p className="mt-4">まずは<Link href="/lp/contact" className="text-sky-600 underline hover:text-sky-800 font-semibold">無料相談</Link>で、貴院の現状リピート率の診断と改善シミュレーションをお試しください。Lオペ for CLINICの導入から運用定着まで、専任サポートが伴走いたします。</p>
      </section>
    </ArticleLayout>
  );
}
