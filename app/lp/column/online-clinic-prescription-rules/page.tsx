  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="ガイド" keyPoints={keyPoints} toc={toc}>
      ...
      <p className="text-[15px] leading-relaxed text-gray-700 font-medium bg-blue-50 rounded-xl p-5 border border-blue-100">
        オンライン診療で処方できる薬・できない薬の境界は、初診か再診か、保険診療か自費診療かによって大きく変わります。本記事では厚労省「オンライン診療の適切な実施に関する指針」に基づき、<strong>薬剤カテゴリごとの処方制限</strong>を網羅的に整理します。AGA・ED・ピルなど自費診療で人気の薬剤についても個別にルールを解説。法規制の全体像は<Link href="/lp/column/online-clinic-regulations" className="text-sky-600 underline hover:text-sky-800">オンライン診療の法規制ガイド</Link>、開業準備からの情報は<Link href="/lp/column/online-clinic-complete-guide" className="text-sky-600 underline hover:text-sky-800">オンライン診療完全ガイド</Link>もあわせてご覧ください。
      </p>
      ...
      <section>
        ...
        <p>
          この制限はクリニック経営にも大きく影響します。たとえば、不眠症を訴える新規患者がオンラインで初診を受けた場合、ゾルピデムやスボレキサント（ベルソムラ）などの処方が初診では行えません。初回は対面診療に誘導するか、漢方薬など向精神薬に該当しない代替薬を提案する運用が求められます。不眠症オンライン診療の詳細は<Link href="/lp/column/insomnia-online-clinic-lope" className="text-sky-600 underline hover:text-sky-800">不眠症オンライン診療ガイド</Link>もご参照ください。
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          処方時の注意点としては、<strong>肝機能障害の有無</strong>の確認が重要です。フィナステリド・デュタステリドは肝臓で代謝されるため、肝疾患のある患者には慎重投与が必要です。また、ミノキシジル内服は血圧低下のリスクがあるため、循環器系の既往歴を問診で確認します。定期的な血液検査（PSA値、肝機能等）のフォローも推奨されます。AGA特化のクリニック戦略については<Link href="/lp/column/aga-online-clinic-winning-strategy" className="text-sky-600 underline hover:text-sky-800">AGA治療オンラインクリニックの勝ち方</Link>をご参照ください。
        </p>
        ...
        <p>
          硝酸薬（ニトログリセリン等）を使用中の患者には<strong>絶対禁忌</strong>です。併用により致死的な血圧低下を引き起こす可能性があるため、問診で硝酸薬の使用有無を必ず確認します。また、α遮断薬や降圧薬との併用にも注意が必要です。ED治療のオンライン診療については<Link href="/lp/column/ed-online-clinic-winning-strategy" className="text-sky-600 underline hover:text-sky-800">ED治療オンラインクリニックの戦略ガイド</Link>も参考になります。
        </p>
        ...
        <p>
          問診では<strong>血栓症の既往、家族歴、喫煙歴、BMI、片頭痛の有無</strong>を必ず確認します。初回処方時は1シート（1か月分）の処方にとどめ、問題がなければ再診で3シートまで処方日数を延ばすフローが安全です。ピルのオンライン処方の詳細は<Link href="/lp/column/pill-online-clinic-winning-strategy" className="text-sky-600 underline hover:text-sky-800">ピルオンラインクリニックの運営戦略</Link>をご覧ください。
        </p>
        ...
        <p>
          処方時の注意点として、<strong>膵炎の既往歴</strong>がある患者には禁忌です。甲状腺髄様癌の家族歴も確認が必要です。また、嘔気・下痢などの消化器系副作用が高頻度で発生するため、低用量（3mg）からの開始と段階的な増量を指導します。GLP-1オンライン処方の詳細は<Link href="/lp/column/diet-online-clinic-winning-strategy" className="text-sky-600 underline hover:text-sky-800">ダイエットオンラインクリニックの勝ち方</Link>をご参照ください。
        </p>

        <h3 className="text-lg font-bold text-gray-700 mt-6">美容内服（トラネキサム酸・ビタミンC等）</h3>
        <p>
          美容内服薬は<strong>初診から処方可能</strong>で、もっともリスクの低い薬剤カテゴリです。トラネキサム酸（肝斑治療）、シナール（ビタミンC・パントテン酸）、ハイチオール（L-システイン）、ユベラ（ビタミンE）など、副作用が少なく安全性の高い薬剤が中心です。美容内服の取り扱いについては<Link href="/lp/column/beauty-supplements-online-lope" className="text-sky-600 underline hover:text-sky-800">美容内服オンライン診療ガイド</Link>もご覧ください。
        </p>
        <p>
        ...
        <p>
          フェキソフェナジン（アレグラ）、ビラスチン（ビラノア）、デスロラタジン（デザレックス）などの第二世代抗ヒスタミン薬は<strong>初診から処方可能</strong>です。花粉症のオンライン診療は季節性の需要が大きく、毎年2〜4月に集患のピークを迎えます。花粉症クリニックの詳細は<Link href="/lp/column/hay-fever-online-clinic-lope" className="text-sky-600 underline hover:text-sky-800">花粉症オンライン診療ガイド</Link>をご参照ください。
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          クリニック側の導入にあたっては、<strong>電子カルテの対応状況</strong>と<strong>HPKIカード（医師資格証）の取得</strong>が前提条件です。初期費用は数十万円程度で、ランニングコストも月額数千円〜1万円程度に収まることが多いです。オンライン診療の運営コスト全般については<Link href="/lp/column/online-medical-cost" className="text-sky-600 underline hover:text-sky-800">オンライン診療の費用と運用コスト</Link>で詳しく解説しています。
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          処方ルールを正しく理解することは、患者の安全を守るだけでなく、クリニックを法的リスクから防衛し、効率的な診療フローを構築する基盤となります。法規制の全体像は<Link href="/lp/column/online-clinic-regulations" className="text-sky-600 underline hover:text-sky-800">オンライン診療の法規制ガイド</Link>、プラットフォーム選びは<Link href="/lp/column/online-clinic-platform-comparison" className="text-sky-600 underline hover:text-sky-800">オンライン診療プラットフォーム比較</Link>もあわせてご確認ください。お問い合わせは<Link href="/lp/contact" className="text-sky-600 underline hover:text-sky-800">こちら</Link>から。
        </p>
      </section>
