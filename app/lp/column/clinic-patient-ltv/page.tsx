    ...
    <ArticleLayout slug={self.slug} breadcrumbLabel="マーケティング" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <Callout type="info" title="LTV思考が経営を変える">
          「初回だけ来てくれればいい」という集患偏重の考え方から、「いかに長く通い続けてもらうか」というLTV思考に切り替えることで、安定した経営基盤を築けます。セグメント配信を活用したリピート促進は<Link href="/lp/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">セグメント配信でリピート率を向上させる方法</Link>で詳しく解説しています。
        </Callout>
      </section>
      ...
      <section>
        ...
        <p>LTVを構成する3つの要素のうち、LINE公式アカウントで直接的にアプローチできるのは「年間来院回数（リピート率）」と「平均通院年数（離脱防止）」です。KPIの管理方法は<Link href="/lp/column/clinic-kpi-dashboard" className="text-sky-600 underline hover:text-sky-800">LINEダッシュボードで見るべきKPI7選</Link>も参考にしてください。</p>
      </section>
      ...
      <section>
        ...
        <p>平均診療単価を向上させるには、既存患者に対する関連メニューの提案（クロスセル）や上位メニューの提案（アップセル）が有効です。患者離脱を防ぎつつ単価を上げる方法は<Link href="/lp/column/clinic-patient-retention" className="text-sky-600 underline hover:text-sky-800">患者離脱防止の方法</Link>も合わせてご覧ください。</p>
      ...
      <section>
        ...
        <Callout type="info" title="パレートの法則がクリニック経営にも当てはまる">
          多くのクリニックでは、上位20%の患者が売上の60〜80%を占めています。VIP患者のフォローに注力することで、限られたリソースで最大の効果を得られます。セグメント配信の詳しい方法は<Link href="/lp/column/segment-delivery-repeat" className="text-sky-600 underline hover:text-sky-800">セグメント配信でリピート率を向上させる方法</Link>をご覧ください。
        </Callout>
      </section>
