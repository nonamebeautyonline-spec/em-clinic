...
const faqItems = [
  { q: "オンライン診療のシステム利用料の設計を始めるために必要な準備は何ですか？", a: "厚生労働省のオンライン診療ガイドラインに基づく届出、ビデオ通話システムの導入、オンライン決済の設定が必要です。Lオペ for CLINICならLINEビデオ通話・電話音声通話でのオンライン診療に対応しており、別途システム導入が不要です。" },
  ...
  return (
    <ArticleLayout slug={self.slug} breadcrumbLabel="経営戦略" keyPoints={keyPoints} toc={toc}>
      ...
      <section>
        ...
        <p>名称はクリニックによって異なり、「通信費」「オンライン診療手数料」「予約管理費」「情報通信機器使用料」など様々な呼び方があります。名称に関わらず、患者にとっては「対面では払わなかった追加費用」として認識されるため、その根拠と金額の妥当性を明確に説明できることが重要です。オンライン診療全体の<Link href="/lp/column/online-clinic-pricing-breakdown" className="text-sky-600 underline hover:text-sky-800">料金相場と費用構造の全体像</Link>を把握した上で設計に臨みましょう。</p>
      ...
      <section>
        ...
        <p>ただし、徴収にあたっては以下の条件を満たす必要があります。まず、患者に対して<strong>事前に金額と徴収理由を説明し、同意を得る</strong>こと。次に、金額が社会通念上妥当な範囲であること。そして、保険診療の場合は保険点数に含まれない費用として、<strong>保険外併用療養費の枠組みでの徴収</strong>とすることです。<Link href="/lp/column/online-clinic-first-visit-revisit-rules" className="text-sky-600 underline hover:text-sky-800">初診・再診の算定ルール</Link>と合わせて理解しておくと、保険請求との整合性を保ちやすくなります。</p>
      ...
      <section>
        ...
        <p>「定額型」や「サブスク型」は、AGA治療やピル処方など<strong>月次の定期処方</strong>に適したモデルです。「月額○円ですべて込み（診察・薬・配送）」と提示できるため、患者は追加費用を心配せずに継続利用できます。<Link href="/lp/column/self-pay-clinic-pricing-strategy" className="text-sky-600 underline hover:text-sky-800">自費クリニックの価格設定戦略</Link>も併せて参考にしてください。クリニック側にとっても月間の売上が予測しやすく、キャッシュフローが安定するメリットがあります。</p>
      </section>
