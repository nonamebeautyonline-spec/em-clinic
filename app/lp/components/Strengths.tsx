...
export default function Strengths() {
  const cards = [
    ...
    { num: "03", title: "クリニック専用設計", sub: "vs. Lステップ・Liny等の汎用ツール", points: ["Lステップ・Linyなどの汎用LINE配信ツールは飲食・EC向け。医療特有の「問診→予約→診療→処方→フォロー」導線に未対応", "患者CRM・対応マーク・処方管理・配送追跡まで、クリニック業務フローに完全特化", "友だち追加→自動問診→予約→来院→決済→配送→フォローまで1画面で追跡。データ分断ゼロ"] },
    { num: "04", title: "すべてがLINEで自動化", sub: "通知も業務連携もすべて自動", points: ["予約確定・リマインド・決済完了・発送通知・追跡番号…すべてのステータス変化がLINEに自動通知。電話・メールの取りこぼしゼロ", "患者はLINEを見るだけで自分の状況を把握。問い合わせ電話が激減し、受付業務を大幅削減", "リッチメニュー・配信・自動アクションもGUIで即設定。管理画面から院長自身がすぐ反映"] },
    { num: "05", title: "スマホでどこからでも管理", sub: "LINE通知bot + モバイル管理画面", points: ["LINE通知botが予約・決済・AI返信の状況をリアルタイムにプッシュ通知。外出先でも状況を即把握", "スマホのブラウザからトーク画面を操作し、AI返信の承認・メッセージ送信・決済確認が可能", "PCがなくても患者対応が止まらない。休憩中・移動中でもワンタップで対応完了"] },
  ];
  ...
  return (
    <Section id="strengths" className="bg-slate-900 text-white">
      <div className="text-center">
        ...
        <h2 className="mb-5 text-[1.7rem] font-extrabold leading-snug tracking-tight text-white md:text-4xl">Lオペ for CLINIC が選ばれる<br className="md:hidden" />5つの理由</h2>
