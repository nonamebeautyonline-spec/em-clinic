...
describe("カルテテンプレートCRUD API (karte-templates/route.ts)", () => {
  beforeEach(() => {
    ...
    mockChain.then = vi.fn((resolve: (val: unknown) => unknown) => resolve({ data: [], error: null, count: 0 }));
  });
  ...
  describe("GET: テンプレート一覧取得", () => {
    ...
    it("テンプレートが存在 → 正常取得", async () => {
      ...
      mockChain.then = vi.fn((resolve: (val: unknown) => unknown) => resolve({ data: templates, error: null }));
    ...
    it("テンプレート0件 → デフォルト8個返却", async () => {
      mockChain.then = vi.fn((resolve: (val: unknown) => unknown) => resolve({ data: [], error: null }));
    ...
    it("テーブル未存在エラー → デフォルト8個返却", async () => {
      mockChain.then = vi.fn((resolve: (val: unknown) => unknown) => resolve({
        data: null,
    ...
    it("data=null → デフォルト返却", async () => {
      mockChain.then = vi.fn((resolve: (val: unknown) => unknown) => resolve({ data: null, error: null }));
  ...
  describe("デフォルトテンプレートの品質", () => {
    ...
    it("デフォルトテンプレートのカテゴリが正しい", async () => {
      ...
      const categories = json.templates.map((t: Record<string, unknown>) => t.category);
