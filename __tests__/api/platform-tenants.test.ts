...
describe("GET /api/platform/tenants", () => {
  ...
  describe("認証チェック", () => {
    it("認証失敗時は403を返す", async () => {
      vi.mocked(verifyPlatformAdmin).mockResolvedValue(null as never);
  ...
  describe("正常系", () => {
    it("テナント一覧を取得できる", async () => {
      ...
      vi.mocked(supabaseAdmin.rpc)
        .mockResolvedValueOnce({ data: [{ tenant_id: "t1", count: 100 }, { tenant_id: "t2", count: 50 }] } as never)
        .mockResolvedValueOnce({ data: [{ tenant_id: "t1", total: 500000 }, { tenant_id: "t2", total: 300000 }] } as never);
    ...
    it("ステータスフィルター: active のみ", async () => {
      ...
      const activeCall = eqCalls.find((c: unknown[]) => c[0] === "is_active" && c[1] === true);
    ...
    it("ステータスフィルター: inactive のみ", async () => {
      ...
      const eqCalls = tenantsChain.eq.mock.calls;
      const inactiveCall = eqCalls.find((c: unknown[]) => c[0] === "is_active" && c[1] === false);
    ...
    it("patients_count ソートが正しく動作する", async () => {
      ...
      vi.mocked(supabaseAdmin.rpc)
        .mockResolvedValueOnce({ data: [{ tenant_id: "t1", count: 10 }, { tenant_id: "t2", count: 100 }] } as never)
  ...
  describe("エラー系", () => {
    it("DBエラー時は500を返す", async () => {
      ...
      const tenantsChain = createChain({ data: null, error: { message: "DB error" }, count: null } as never);
      tableChains["tenants"] = tenantsChain;
  ...
  describe("RPCフォールバック", () => {
    it("RPC未定義時は個別カウントにフォールバックする", async () => {
      ...
      const ordersChain = createChain({ data: [{ amount: 10000 }, { amount: 20000 }], error: null } as never);
...
describe("POST /api/platform/tenants", () => {
  ...
  beforeEach(() => {
    ...
    // デフォルト: parseBody成功
    vi.mocked(parseBody).mockResolvedValue({ data: validTenantData } as never);
  });
  ...
  describe("slug重複チェック", () => {
    it("slug重複時は409を返す", async () => {
      ...
      // slug検索: 既存あり
      const tenantsChain = createChain({ data: { id: "existing-id" }, error: null } as never);
      tableChains["tenants"] = tenantsChain;
    ...
    it("テナント作成に成功すると201を返す", async () => {
      ...
      tenantsChain.maybeSingle = vi.fn().mockImplementation(() => {
        ...
        if (tenantsCallCount <= 1) {
          // slug検索: 存在しない
          return { then: (fn: (val: unknown) => unknown) => fn({ data: null, error: null }) };
        }
        return tenantsChain;
      });
      // singleの時は「テナントINSERT結果」
      tenantsChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: { id: "new-tenant-id" }, error: null }),
      });
      tableChains["tenants"] = tenantsChain;
      ...
      adminUsersChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: { id: "new-admin-id", username: "LP-A1B2C" }, error: null }),
      });
      tableChains["admin_users"] = adminUsersChain;

      // テナントメンバー
      const memberChain = createChain({ data: null, error: null } as never);
      tableChains["tenant_members"] = memberChain;

      // プラン
      const planChain = createChain({ data: null, error: null } as never);
  ...
  describe("テナント作成失敗", () => {
    it("テナントINSERT失敗時は500を返す", async () => {
      ...
      tenantsChain.maybeSingle = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: null, error: null }),
      });
      tenantsChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: null, error: { message: "insert failed" } }),
      });
      tableChains["tenants"] = tenantsChain;
  ...
  describe("管理者ユーザー作成失敗時のロールバック", () => {
    it("管理者作成失敗時はテナントを削除してロールバックする", async () => {
      ...
      adminUsersChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: null, error: { message: "admin insert failed" } }),
      });
      tableChains["admin_users"] = adminUsersChain;
  ...
  describe("LINE設定保存", () => {
    it("LINE設定がある場合はtenant_settingsに保存される", async () => {
      ...
      vi.mocked(parseBody).mockResolvedValue({ data: dataWithLine } as never);
      ...
      tenantsChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: { id: "t-new" }, error: null }),
      });
      tableChains["tenants"] = tenantsChain;
      ...
      adminUsersChain.single = vi.fn().mockReturnValue({
        then: (fn: (val: unknown) => unknown) => fn({ data: { id: "a-new", username: "LP-X1Y2Z" }, error: null }),
      });
      tableChains["admin_users"] = adminUsersChain;
      ...
      const planChain = createChain({ data: null, error: null } as never);
      tableChains["tenant_plans"] = planChain;
      const settingsChain = createChain({ data: null, error: null } as never);
