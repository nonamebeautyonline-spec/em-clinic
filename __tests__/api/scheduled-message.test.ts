...
describe("トーク画面: 予約送信UI", () => {
  // TalkClient分割後: state → useTalkState, ハンドラ → useMessageHandlers, UI → MessageColumn
  const stateFile = "app/admin/line/talk/_components/talk/useTalkState.ts";
  const handlersFile = "app/admin/line/talk/_components/talk/useMessageHandlers.ts";
  const uiFile = "app/admin/line/talk/_components/talk/MessageColumn.tsx";

  it("scheduleMode state が定義されている", () => {
    const src = readFile(stateFile);
  ...
  it("予約送信メッセージ取得関数がある", () => {
    const src = readFile(handlersFile);
...
describe("予約送信: 日時バリデーション", () => {
  it("最小日時は現在時刻+5分", () => {
    const src = readFile("app/admin/line/talk/_components/talk/MessageColumn.tsx");
    // UIのmin属性で5分先を設定
