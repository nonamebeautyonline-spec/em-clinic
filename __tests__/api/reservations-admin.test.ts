...
// --- チェーン生成ヘルパー ---
function createChain(defaultResolve = { data: [], error: null, count: 0 }): MockChain {
...
describe("管理者用予約一覧API (admin/reservations/route.ts)", () => {
  ...
  describe("デフォルト: 今日の予約取得", () => {
    it("パラメータ無し → 今日の予約を取得", async () => {
      reservationsChain.then = vi.fn((resolve: (value: unknown) => unknown) => resolve({
        data: [
          {
            id: "r1",
            reserve_id: "res-001",
      ...
      patientsChain.then = vi.fn((resolve: (value: unknown) => unknown) => resolve({
        data: [
      ...
      intakeChain.then = vi.fn((resolve: (value: unknown) => unknown) => resolve({
        data: [
