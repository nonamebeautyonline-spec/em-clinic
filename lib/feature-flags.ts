// lib/feature-flags.ts — プラン別機能フラグ（サーバー専用: supabaseAdmin依存）
//
// 従量課金制移行後:
// - 全プランで BASE_FEATURES が利用可（プラン間の機能差なし）
// - AI系3機能は tenant_options テーブルで個別課金管理
//
// ⚠️ クライアントコンポーネントからは lib/feature-flags-shared.ts をインポートすること
//    （このファイルは supabaseAdmin を含むためクライアントバンドルに混入するとエラーになる）
import { supabaseAdmin } from "@/lib/supabase";

// 型定義・定数は共有ファイルから re-export
export type { Feature, Industry } from "@/lib/feature-flags-shared";
export {
  INDUSTRY_LABELS,
  PRODUCT_NAMES,
  INDUSTRY_ICONS,
