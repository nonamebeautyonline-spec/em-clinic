...
describe("invoice-pdf", () => {
  ...
  it("支払日が表示される（入金済みの場合）", () => {
    generateInvoicePDF(sampleData);
    const textCalls = mockText.mock.calls.map((c: unknown[]) => c[0]);
    expect(textCalls.some((t) => typeof t === "string" && t.includes("Paid At"))).toBe(true);
  });

  it("未入金の場合は支払日が表示されない", () => {
    generateInvoicePDF({ ...sampleData, paidAt: null });
    const textCalls = mockText.mock.calls.map((c: unknown[]) => c[0]);
    expect(textCalls.some((t) => typeof t === "string" && t.includes("Paid At"))).toBe(false);
  });

  it("備考が表示される", () => {
    generateInvoicePDF(sampleData);
    const textCalls = mockText.mock.calls.map((c: unknown[]) => c[0]);
    expect(textCalls.some((t) => typeof t === "string" && t.includes("Notes:"))).toBe(true);
  });

  it("備考なしの場合は備考行が表示されない", () => {
    generateInvoicePDF({ ...sampleData, notes: null });
    const textCalls = mockText.mock.calls.map((c: unknown[]) => c[0]);
    expect(textCalls.some((t) => typeof t === "string" && t.includes("Notes:"))).toBe(false);
  });
