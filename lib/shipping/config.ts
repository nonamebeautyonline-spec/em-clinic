...
export async function getShippingConfig(): Promise<ShippingConfig> {
  const raw = await getSetting("general", SETTING_KEY);
...
/** DB に配送設定を保存 */
export async function setShippingConfig(config: Partial<ShippingConfig>): Promise<boolean> {
  ...
  return setSetting("general", SETTING_KEY, JSON.stringify(merged));
}

/** ヤマト設定のみ取得（フォーマッター向け） */
export async function getYamatoConfig(): Promise<YamatoConfig> {
