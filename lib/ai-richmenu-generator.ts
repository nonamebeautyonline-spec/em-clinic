...
- 配置パターンは2種類。プロンプトの雰囲気に応じて選択:
  - **パターンA（アイコン上・ラベル下）**: アイコン中心Y = セル上端 + H×0.33、ラベルY = セル上端 + H×0.72、サブラベルY = セル上端 + H×0.84
  - **パターンB（ラベル上・アイコン下）**: ラベルY = セル上端 + H×0.25、サブラベルY = セル上端 + H×0.37、アイコン中心Y = セル上端 + H×0.65
- アイコンの最大サイズ: セル高さの 25% 以内（H×0.25）
- ラベルのfont-size: メイン 44-56px、サブ 26-34px
- **アイコンとラベルの間に最低 80px の空白を確保**
...
- カード内にアイコン＋メインラベル（太字）＋サブラベル（英語/小さめ）
...
<rect x="25" y="25" width="793" height="803" rx="16" fill="white" stroke="#D4C5B0" stroke-width="2"/>

### 二重枠型カード（高級系）
<rect x="25" y="25" width="793" height="803" rx="8" fill="white" stroke="#C4A265" stroke-width="1.5"/>
<rect x="38" y="38" width="767" height="777" rx="6" fill="none" stroke="#C4A265" stroke-width="1"/>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2500 1686">
  ...
  <!-- 背景テクスチャ（水彩風の例） -->
  <circle cx="400" cy="300" r="350" fill="[背景色やや濃い]" opacity="0.08"/>
  <circle cx="2100" cy="1400" r="400" fill="[背景色やや濃い]" opacity="0.06"/>

  <!-- カード1 (左上, カード高803px) -->
  <g filter="url(#shadow)">
    <rect x="25" y="25" width="793" height="803" rx="24" fill="white"/>
  </g>
  <!-- アイコン（カード上端25 + 803×0.33 = 290） -->
  <g transform="translate(421,290)">
    <!-- アイコンSVG -->
  </g>
  <!-- メインラベル（カード上端25 + 803×0.72 = 603） -->
  <text x="421" y="603" text-anchor="middle" fill="#1F2937" font-size="52" font-weight="bold" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">予約</text>
  <!-- サブラベル（カード上端25 + 803×0.84 = 699） -->
  <text x="421" y="699" text-anchor="middle" fill="#9CA3AF" font-size="30" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">RESERVATION</text>
...
## SVGテンプレート（パターンB: ラベル上・アイコン下）
  ...
  <!-- カード内: メインラベル（カード上端 + H×0.25） -->
  <text x="421" y="226" text-anchor="middle" fill="#1F2937" font-size="48" font-weight="bold" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">予約</text>
  <!-- カード内: サブラベル（カード上端 + H×0.37） -->
  <text x="421" y="322" text-anchor="middle" fill="#9CA3AF" font-size="28" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">RESERVATION</text>
  <!-- カード内: アイコン（カード上端 + H×0.65） -->
  <g transform="translate(421,547)">
    <!-- アイコンSVG -->
  </g>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2500 1686">
  ...
  <!-- セル1: アイコン（セル上端8 + 831×0.33 = 282） -->
  <g transform="translate(416,282)">
    <circle cx="0" cy="0" r="65" fill="none" stroke="white" stroke-width="5"/>
  </g>
  <!-- セル1: メインラベル（セル上端8 + 831×0.72 = 606） -->
  <text x="416" y="606" text-anchor="middle" fill="white" font-size="52" font-weight="bold" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">ラベル1</text>
  <!-- セル1: サブラベル（セル上端8 + 831×0.84 = 706） -->
  <text x="416" y="706" text-anchor="middle" fill="rgba(255,255,255,0.7)" font-size="28" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">LABEL</text>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2500 1686">
  ...
  <!-- ボタンカード（3つ横並び, カード高636px） -->
  <g filter="url(#card-shadow)">
    <rect x="25" y="1025" width="800" height="636" rx="20" fill="white"/>
  </g>
  <!-- カード1: ラベル（カード上端1025 + 636×0.25 = 1184） -->
  <text x="425" y="1184" text-anchor="middle" fill="#1F2937" font-size="44" font-weight="bold" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">ラベル</text>
  <text x="425" y="1230" text-anchor="middle" fill="#9CA3AF" font-size="26" font-family="'Hiragino Sans','Yu Gothic','Noto Sans JP',sans-serif">LABEL</text>
  <!-- カード1: アイコン（カード上端1025 + 636×0.65 = 1438） -->
  <g transform="translate(425,1438)">
    <!-- アイコン -->
  </g>
