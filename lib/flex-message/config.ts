...
export async function getFlexConfig(tenantId?: string): Promise<FlexMessageConfig> {
  ...
  try {
    ...
    return {
      ...
      reservationColors: parsed.reservationColors ? { ...DEFAULT_FLEX_CONFIG.colors, ...parsed.reservationColors } : undefined,
      shippingColors: parsed.shippingColors ? { ...DEFAULT_FLEX_CONFIG.colors, ...parsed.shippingColors } : undefined,
      paymentColors: parsed.paymentColors ? { ...DEFAULT_FLEX_CONFIG.colors, ...parsed.paymentColors } : undefined,
      paymentThankColors: parsed.paymentThankColors ? { ...DEFAULT_FLEX_CONFIG.colors, ...parsed.paymentThankColors } : undefined,
...
export async function setFlexConfig(config: Partial<FlexMessageConfig>, tenantId?: string): Promise<boolean> {
  ...
  const merged: FlexMessageConfig = {
    ...
    reservationColors: config.reservationColors ? { ...(current.reservationColors ?? current.colors), ...config.reservationColors } : current.reservationColors,
    shippingColors: config.shippingColors ? { ...(current.shippingColors ?? current.colors), ...config.shippingColors } : current.shippingColors,
    paymentColors: config.paymentColors ? { ...(current.paymentColors ?? current.colors), ...config.paymentColors } : current.paymentColors,
    paymentThankColors: config.paymentThankColors ? { ...(current.paymentThankColors ?? current.colors), ...config.paymentThankColors } : current.paymentThankColors,
