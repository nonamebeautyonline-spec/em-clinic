...
export interface IntakeFormField {
  ...
  ng_block?: boolean;
  ng_block_value?: string;
  ng_block_message?: string;
}
...
export interface IntakeFormSettings {
  ...
  ng_block_title?: string;
  ng_block_message?: string;
}

// のなめクリニックの現行問診フォーム（フォールバック用）
export const DEFAULT_INTAKE_FIELDS: IntakeFormField[] = [
  {
    id: "ng_check",
    type: "radio",
...
export const DEFAULT_INTAKE_SETTINGS: IntakeFormSettings = {
  step_by_step: true,
  header_title: "問診",
  estimated_time: "平均回答時間 1〜2分程度",
  ng_block_title: "オンライン処方の対象外です",
  ng_block_message:
    "恐れ入りますが、問診項目のいずれかに該当する場合はオンラインでの処方ができかねます。お手数ですが、対面診療が可能な医療機関でのご相談をご検討ください。",
};
