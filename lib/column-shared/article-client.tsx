...
/* ═══════════════════════════════════════════════════════════════════════════
   SNSシェアボタン（siteBaseUrl + columnPathでURL生成）
   ═══════════════════════════════════════════════════════════════════════════ */

export function ShareButtons({ title, slug, siteBaseUrl = "https://l-ope.jp", columnPath = "/lp/column" }: {
  title: string;
  slug: string;
  siteBaseUrl?: string;
  columnPath?: string;
}) {
  const url = `${siteBaseUrl}${columnPath}/${slug}`;
...
/* ═══════════════════════════════════════════════════════════════════════════
   InlineCTAボタン（パス・テキストをパラメータ化）
   ═══════════════════════════════════════════════════════════════════════════ */

export function InlineCTAButton({ contactPath = "/lp/contact", columnPathPrefix = "/lp/column/", label = "Lオペの資料を無料で請求" }: {
  contactPath?: string;
  columnPathPrefix?: string;
  label?: string;
}) {
  const pathname = usePathname();
  const slug = pathname.replace(new RegExp(`^${columnPathPrefix.replace(/\//g, "\\/")}`), "").replace(/\/$/, "") || "column";
  return (
    <a
      href={`${contactPath}?ref=${slug}`}
