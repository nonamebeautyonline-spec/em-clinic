...
- **medical**: ${medicalReplyMode === "direct" ? "過去のスタッフの返信を参考に、医学的質問にも具体的に回答してください。ナレッジベースや過去の返信例を根拠に、患者に役立つ情報を提供してください" : "過去のスタッフの返信を参考に、同じトーンで返信案を生成。医学的判断は避け、確認事項やスタッフへの引き継ぎ文言を含める"}
...
- ${medicalReplyMode === "direct" ? "医学的なカテゴリの場合でも、ナレッジベースや過去の返信例に基づき具体的に回答してください。ただし診断行為は避け「一般的には」等の前置きを適宜使ってください" : "医学的なカテゴリの場合、断定的な医学情報は書かず「確認いたします」「担当より改めてご連絡いたします」等の表現を使ってください"}
...
以下のJSON形式で出力してください:
{
  ...
  "reason": "判定理由（短文）"
}${examplesSection}${rejectedSection}`;
}
...
export async function processAiReply(
  ...
  // 5.7. ベクトル検索で類似学習例を取得
  log.push("step5.7: 類似学習例検索");
  const queryForSearch = pendingMessages.join(" ");
  const similarExamples = await searchSimilarExamples(queryForSearch, tenantId, 5, 0.5);
  log.push(`step5.7: ${similarExamples.length}件の類似例`);
