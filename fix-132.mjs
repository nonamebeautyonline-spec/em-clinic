...
console.log('=== Patient 20260100132 を補完 ===\n');

// GASからデータ取得
const gasRes = await fetch(GAS_INTAKE_URL);
const gasData = await gasRes.json();
const gasRows = gasData.ok ? gasData.rows : gasData;
const row = gasRows.find(r => String(r.patient_id || '').trim() === '20260100132');
...
if (ADMIN_TOKEN && APP_BASE_URL) {
  await fetch(`${APP_BASE_URL}/api/admin/invalidate-cache`, {
    ...
    body: JSON.stringify({ patient_id: '20260100132' })
  });
