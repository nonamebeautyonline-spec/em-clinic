# Lオペ for CLINIC — アプリケーション仕様書【社内機密】
...
| 音声認識 | Deepgram Nova-2（メイン）/ Groq Whisper-Turbo（フォールバック） |
| エラー監視 | Sentry |
| デプロイ | Vercel |
...
| パス | 役割 |
|------|------|
| `/doctor` | カルテ画面（3タブ: 本日の予約・新規予約・検索。AIカルテ生成対応） |
...
| `/admin/dashboard` | ダッシュボード詳細（売上・予約・配送統計・患者セグメント分析） |
...
| パス | 役割 |
|------|------|
| `/admin/patients/[patientId]` | 患者個別ページ（詳細情報、メモ、タグ、AIカルテ生成） |
...
| `/admin/doctor` | 医師用カルテ画面（管理者版、3タブ構成、AIカルテ生成対応） |
...
| `/api/admin/segments` | GET | 患者セグメント取得（VIP/アクティブ/離脱リスク/休眠/新規） |
| `/api/admin/segments/recalculate` | POST | 患者セグメント再計算 |
...
| ページ | 内容 |
|--------|------|
| `login` | ログイン画面（localStorage認証） |
| `page`（トップ） | ダッシュボード（KPI・グラフ・通知・セグメント） |
| `talk` | LINEトーク（メッセージ送受信・テンプレート・自動返信） |
| `friends` | 友だち管理（検索・タグフィルタ・詳細パネル） |
| `tags` | タグ管理（作成・編集・色設定） |
| `templates` | メッセージテンプレート（カテゴリ別・変数置換） |
| `richmenu` | リッチメニュー管理（ビジュアルエディタ） |
| `broadcasts` | メッセージ配信（作成・LINEプレビュー・履歴・レポート） |
| `keyword-replies` | キーワード自動返信（ルール設定・テスト） |
| `ai-reply` | AI自動返信設定（モデル・トーン・除外条件） |
| `calendar` | 予約カレンダー（月間表示・予約一覧） |
| `karte` | Drカルテ（3タブ・処方モーダル・AIカルテ生成） |
| `reorders` | 再処方管理（申請一覧・承認/却下） |
| `shipping` | 発送管理（ステータス・追跡番号付与） |
| `analytics` | 分析（売上推移・コンバージョン・セグメント） |
| `products` | 商品マスタ（価格・在庫・有効/無効切替） |
| `settings` | 設定（クリニック情報・LINE連携・AI設定） |
