// clear-intake-caches.mjs
// problematic-intake-patient-ids.json に記載された患者のキャッシュを一括クリア
...
console.log("=== 問診送信済み患者のキャッシュを一括クリア ===\n");

// 患者IDリストを読み込み
let patientIds;
try {
  const data = fs.readFileSync("problematic-intake-patient-ids.json", "utf8");
  patientIds = JSON.parse(data);
} catch (err) {
  console.error("❌ problematic-intake-patient-ids.json が見つかりません");
  console.error("   先に find-intake-cache-issues.mjs を実行してください");
  process.exit(1);
}
